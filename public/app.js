...
function setReferralProgramMessage(message = '', tone = 'info') {
    ...
    if (!message) {
        ...
        container.className = 'mt-4 hidden rounded-[20px] border px-4 py-4 text-sm font-semibold';
        return;
    }
    const toneMap = {
    ...
    container.className = `mt-4 rounded-[20px] border px-4 py-4 text-sm font-semibold ${toneMap[tone] || toneMap.info}`;
...
function renderReferralActivityCard(entry, variant = 'outgoing') {
    ...
    const flagsMarkup = Array.isArray(entry.fraudFlags) && entry.fraudFlags.length
        ? `<div class="mt-3 text-xs font-semibold text-slate-500">Flags: ${escapeHtml(entry.fraudFlags.join(', '))}</div>`
        : '';
    const reasonMarkup = entry.blockedReason
        ? `<div class="mt-3 rounded-2xl border border-rose-200 bg-rose-50 px-3 py-3 text-sm font-semibold text-rose-700">${escapeHtml(entry.blockedReason)}</div>`
        : '';
    return `
        <div class="rounded-[20px] border border-slate-200 bg-slate-50 p-4">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-500">${escapeHtml(activityLabel)}</div>
                    <div class="mt-2 text-base font-semibold text-slate-900">${escapeHtml(primaryName)}</div>
                    <div class="mt-1 text-sm text-slate-500">${escapeHtml(secondaryText)}</div>
                </div>
                ${renderStatusBadge(entry.status || 'pending')}
            </div>
            <div class="mt-4 grid gap-3 sm:grid-cols-3">
                <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-500">Qualifying Deposit</div>
                    <div class="mt-2 text-sm font-semibold text-slate-900">${escapeHtml(qualifyingDepositAmount ? formatMoneyPrecise(qualifyingDepositAmount) : 'Pending')}</div>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-500">Bonus</div>
                    <div class="mt-2 text-sm font-semibold text-slate-900">${escapeHtml(bonusValue ? formatMoneyPrecise(bonusValue) : 'Pending')}</div>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-500">Updated</div>
                    <div class="mt-2 text-sm font-semibold text-slate-900">${escapeHtml(formatRelativeTime(entry.bonusIssuedAt || entry.createdAt))}</div>
                </div>
            </div>
            ${reasonMarkup}
            ${flagsMarkup}
...
function openReferralProgram(options = {}) {
    ...
    const section = qs('referral-program-section');
    if (!section) return;
...
function hideReferralProgram() {
    qs('referral-program-section')?.classList.add('hidden');
}
