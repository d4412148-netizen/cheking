...
function getVisibleActiveOrdersForCurrentFilter() {
    ...
    if (state.activationFilter === 'waiting') {
        return activeOrders.filter((order) => getOrderLifecycleStatus(order) === 'pending');
    }
...
function renderCompletedOrders() {
    ...
    container.innerHTML = completedOrders.map((order) => {
        ...
        return `
            <article class="sms-order-card sms-order-card--ready">
...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = activeOrders.map((order) => {
        ...
        const statusValue = isOtpReady ? 'OTP Received' : 'Waiting for OTP';
            ...
            : `
                    <div class="sms-order-live-box">
                        ...
                        <div class="sms-order-live-copy min-w-0">
                            <div class="sms-order-live-title">Waiting for OTP</div>
                        </div>
                    </div>
                `;
        const actionAreaMarkup = isOtpReady
            ...
            `;
        return `
            <article class="sms-order-card ${isOtpReady ? 'sms-order-card--ready' : 'sms-order-card--waiting'}">
                <div class="sms-order-head">
                    <div class="sms-order-brand">
                        ${renderServiceLogo(order.service_type, 'sm')}
                        <div class="sms-order-service truncate">${escapeHtml(meta.label)}</div>
