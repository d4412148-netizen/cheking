...
const PENDING_REFERRAL_CODE_STORAGE_KEY = 'mrf_pending_referral_code_v1';
const DEVICE_FINGERPRINT_STORAGE_KEY = 'mrf_device_fingerprint_v1';
...
function getPendingReferralCode() {
    return normalizeReferralCode(readStorageValue(PENDING_REFERRAL_CODE_STORAGE_KEY));
}

function setPendingReferralCode(referralCode) {
    writeStorageValue(PENDING_REFERRAL_CODE_STORAGE_KEY, normalizeReferralCode(referralCode));
}

function clearPendingReferralCode() {
    writeStorageValue(PENDING_REFERRAL_CODE_STORAGE_KEY, '');
}

function getDeviceFingerprint() {
    const existingFingerprint = readStorageValue(DEVICE_FINGERPRINT_STORAGE_KEY);
    if (existingFingerprint) return existingFingerprint;
    const createdFingerprint = typeof window.crypto?.randomUUID === 'function'
        ? window.crypto.randomUUID()
        : `df-${Date.now()}-${Math.random().toString(36).slice(2, 12)}`;
    writeStorageValue(DEVICE_FINGERPRINT_STORAGE_KEY, createdFingerprint);
    return createdFingerprint;
}

function createDeviceHeaders(headers = {}) {
...
function buildReferralLink(referralCode) {
    ...
    const baseOrigin = /(?:localhost|127\.0\.0\.1)/i.test(window.location.host)
        ? window.location.origin
        : 'https://mrfsms.com';
    return `${baseOrigin.replace(/\/+$/, '')}/signup?ref=${encodeURIComponent(normalizedCode)}`;
}

function resetReferralCopyButton() {
    const copyButton = qs('referral-copy-button');
    if (!copyButton) return;
    copyButton.textContent = 'Copy Link';
    copyButton.classList.remove('is-copied');
    if (referralCopyResetTimer) {
        window.clearTimeout(referralCopyResetTimer);
        referralCopyResetTimer = null;
    }
}

function markReferralLinkCopied() {
    const copyButton = qs('referral-copy-button');
    if (!copyButton) return;
    if (referralCopyResetTimer) {
        window.clearTimeout(referralCopyResetTimer);
    ...
    referralCopyResetTimer = window.setTimeout(() => {
        resetReferralCopyButton();
    }, 2200);
}

function syncReferralLinkInput(link) {
    const input = qs('referral-link-input');
    if (input) {
...
async function openReferralModal() {
    ...
    hideAccountDetails();
    hideHeaderQuickMenu();
    syncReferralLinkInput(buildReferralLink(state.currentUser.referralCode));
    openModal('referral-modal');
    try {
        const referralProgram = await fetchJSON('/api/referral-program', {
            headers: createDeviceHeaders()
        });
        const referralLink = referralProgram?.referralLink || buildReferralLink(referralProgram?.referralCode || state.currentUser?.referralCode);
        syncReferralLinkInput(referralLink);
    } catch (err) {
        syncReferralLinkInput(buildReferralLink(state.currentUser?.referralCode));
        showToast(err.message || 'Could not load referral link', 'error');
    }
}

async function copyReferralLink() {
    const referralLink = String(qs('referral-link-input')?.value || '').trim();
    if (!referralLink) {
...
async function checkAuth() {
    try {
        ...
        hidePaymentTopAlert();
        stopAdminAlertLoop();
        if (state.startupAuthMode) {
            const startupAuthMode = state.startupAuthMode;
            state.startupAuthMode = '';
            openAuthModal(startupAuthMode);
        }
    }
}
...
function showStartupMessages() {
    ...
    const isSignupPath = /^\/signup\/?$/i.test(window.location.pathname);
    ...
    if (referralCode || isSignupPath) {
        state.startupAuthMode = 'register';
    }
    if (shouldUpdateUrl) {
