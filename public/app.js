...
function getGeneratedServiceLogoUrl(label) {
    ...
    const iconSlug = EXTRA_SERVICE_ICON_SLUGS[lookupKey] || lookupKey;
    return iconSlug ? `https://cdn.simpleicons.org/${encodeURIComponent(iconSlug)}` : '';
}

function getServiceLogoFallbackUrl(label, fallback = '') {
    const shortLabel = buildServiceShortLabel(label, fallback);
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(shortLabel)}&size=128&background=ecfdf5&color=166534&rounded=true&bold=true&format=svg`;
}
...
function renderServiceBrandImageGlyph(serviceType) {
    const meta = getServiceMeta(serviceType);
    const logoUrl = meta.logoUrl || serviceBrandLogoUrls[serviceType] || getGeneratedServiceLogoUrl(meta.label || serviceType);
    const fallbackLogoUrl = getServiceLogoFallbackUrl(meta.label, serviceType);
    if (!logoUrl) return '';
    return `
        <span class="relative inline-flex h-full w-full items-center justify-center rounded-[7px] bg-white/95 p-[2px]">
            <img src="${escapeAttr(logoUrl)}" data-fallback-src="${escapeAttr(fallbackLogoUrl)}" alt="${escapeAttr(meta.label)} logo" loading="lazy" referrerpolicy="no-referrer" class="h-full w-full object-contain rounded-[6px]" onerror="if(this.dataset.fallbackSrc && this.dataset.fallbackApplied !== 'true'){this.dataset.fallbackApplied='true';this.src=this.dataset.fallbackSrc;return;}this.classList.add('hidden');this.nextElementSibling.classList.remove('hidden');">
...
function updateActivationSummaryLine() {
    ...
    if (state.activationFilter === 'waiting') {
        summaryLine.textContent = `Waiting for numbers ${state.waitingOrdersExpanded ? '▲' : '▼'}`;
        syncWaitingOrdersVisibility();
        return;
    }
...
function syncWaitingOrdersVisibility() {
    ...
    const hasWaitingOrders = state.orders.some((order) => isWaitingViewOrder(order));
    const isWaitingActivationView = state.historyView === 'activations' && state.activationFilter === 'waiting' && Boolean(state.currentUser);
    const isExpanded = !isWaitingActivationView || !hasWaitingOrders || state.waitingOrdersExpanded;
    summaryLine.classList.toggle('activation-summary-toggle', isWaitingActivationView);
    summaryLine.tabIndex = isWaitingActivationView ? 0 : -1;
    summaryLine.setAttribute('aria-disabled', String(!isWaitingActivationView));
    summaryLine.setAttribute('aria-expanded', String(isExpanded));
    ordersList.classList.toggle('hidden', !isExpanded);
}
...
async function loadCountries() {
    try {
        ...
        state.allCountries = applyFallbackIfEmpty(Array.isArray(countries) ? countries : []);
...
function updateProcessingModalState() {
    ...
    const list = qs('processing-active-list');

    if (summary) {
        summary.textContent = 'Waiting for number';
    }

    if (details) {
        details.textContent = requestCount === 1
            ? '1 number in queue'
            : `${requestCount} numbers in queue`;
    }

    if (list) {
        list.innerHTML = activeRequests.slice(0, 3).map((entry) => `
            <div class="processing-queue-row">
                <span class="processing-queue-spinner" aria-hidden="true"></span>
                <div class="min-w-0">
                    <div class="processing-queue-service truncate">${escapeHtml(getServiceMeta(entry.serviceType).label)}</div>
                    <div class="processing-queue-country truncate">${escapeHtml(entry.countryName)}${Number(entry.price || 0) > 0 ? ` • ${escapeHtml(formatMoney(entry.price))}` : ''}</div>
                </div>
            </div>
        `).join('');
        list.classList.toggle('hidden', requestCount === 0);
    }
...
function bindStaticEvents() {
    ...
    qs('activation-summary-line')?.addEventListener('click', () => {
        const hasWaitingOrders = state.orders.some((order) => isWaitingViewOrder(order));
        if (state.historyView !== 'activations' || state.activationFilter !== 'waiting' || !hasWaitingOrders) return;
        state.waitingOrdersExpanded = !state.waitingOrdersExpanded;
        updateActivationSummaryLine();
        syncWaitingOrdersVisibility();
    });
