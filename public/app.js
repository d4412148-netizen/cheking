...
const LOCAL_DEFAULT_LOGO_URL = '/api/icons/default';
const REFERRAL_CODE_STORAGE_KEY = 'mrf-pending-referral-code-v1';
const DEVICE_FINGERPRINT_STORAGE_KEY = 'mrf-device-fingerprint-v1';
...
function buildBrowserFingerprint() {
    ...
    return browserParts.join('|').slice(0, 255);
}

function getOrCreateDeviceFingerprint() {
    const existing = readStorageValue(DEVICE_FINGERPRINT_STORAGE_KEY);
    if (existing) return existing;
    const generated = typeof window.crypto?.randomUUID === 'function'
        ? window.crypto.randomUUID()
        : `mrf-${Date.now()}-${Math.random().toString(36).slice(2, 12)}`;
    writeStorageValue(DEVICE_FINGERPRINT_STORAGE_KEY, generated);
    return generated;
}

function initializeClientIdentity() {
    state.deviceFingerprint = getOrCreateDeviceFingerprint();
    state.browserFingerprint = buildBrowserFingerprint();
}

function persistPendingReferralCode(referralCode) {
    const normalizedCode = normalizeReferralCode(referralCode);
    state.pendingReferralCode = normalizedCode;
    if (normalizedCode) {
        writeStorageValue(REFERRAL_CODE_STORAGE_KEY, normalizedCode);
    } else {
        removeStorageValue(REFERRAL_CODE_STORAGE_KEY);
    }
    return normalizedCode;
}

function hydratePendingReferralCode() {
    ...
    if (referralFromUrl) {
        ...
        return { code: referralFromUrl, fromUrl: true };
    }
    const storedReferralCode = normalizeReferralCode(readStorageValue(REFERRAL_CODE_STORAGE_KEY));
    if (storedReferralCode) {
        state.pendingReferralCode = storedReferralCode;
    }
    return { code: storedReferralCode, fromUrl: false };
}
...
function getStaticServiceLogoUrl(serviceType) {
    ...
    const lookupValue = String(meta.label || serviceType || '').trim();
    return lookupValue ? `/api/icons/${encodeURIComponent(lookupValue)}` : LOCAL_DEFAULT_LOGO_URL;
}
...
function renderFallbackServiceGlyph(serviceType) {
    ...
    return `<span class="inline-flex h-full w-full items-center justify-center rounded-[10px] bg-[linear-gradient(135deg,#0f172a_0%,#2563eb_100%)] text-[0.68em] font-black uppercase tracking-[0.18em] text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.12)]">${fallbackText}</span>`;
}

function renderServiceBrandImageGlyph(serviceType) {
    const meta = getServiceMeta(serviceType);
    const iconUrl = getStaticServiceLogoUrl(serviceType);
    if (!iconUrl) return '';
    return `
        <span class="service-brand-image-shell">
            <img
                src="${escapeAttr(iconUrl)}"
                alt="${escapeAttr(`${meta.label || serviceType || 'Service'} logo`)}"
                class="service-brand-image"
                loading="lazy"
                decoding="async"
                onerror="if(this.dataset.fallbackApplied==='1'){return;}this.dataset.fallbackApplied='1';this.src='${escapeAttr(LOCAL_DEFAULT_LOGO_URL)}';"
            >
        </span>
    `;
}
...
function formatStatus(status) {
    ...
    if (normalized === 'referral referrer bonus') return 'Referral Bonus';
    if (normalized === 'referral welcome bonus') return 'Welcome Bonus';
    if (normalized === 'bonus issued') return 'Bonus Issued';
    return normalized
        .replace(/_/g, ' ')
...
function setReferralProgramMessage(message = '', tone = 'info') {
    const container = qs('referral-program-message');
    if (!container) return;
    if (!message) {
        container.textContent = '';
        container.className = 'mt-4 hidden rounded-[24px] border px-4 py-4 text-sm font-semibold';
        return;
    }
    const toneMap = {
    ...
    container.className = `mt-4 rounded-[24px] border px-4 py-4 text-sm font-semibold ${toneMap[tone] || toneMap.info}`;
    container.textContent = message;
}

function renderReferralActivityCard(entry, variant = 'outgoing') {
    const isIncoming = variant === 'incoming';
    const primaryName = isIncoming ? (entry.referrerName || 'Referral Partner') : (entry.name || 'Friend');
    const secondaryText = isIncoming ? (entry.referrerEmail || 'Referral bonus status') : (entry.email || 'Pending signup');
    const bonusValue = isIncoming ? Number(entry.referredBonus || 0) : Number(entry.referrerBonus || 0);
    const activityLabel = isIncoming ? 'Welcome Bonus Status' : 'Referral Join';
    const qualifyingDepositAmount = Number(entry.qualifyingDepositAmount || 0);
    const flagsMarkup = Array.isArray(entry.fraudFlags) && entry.fraudFlags.length
        ? `<div class="mt-3 text-xs font-semibold text-slate-300">Flags: ${escapeHtml(entry.fraudFlags.join(', '))}</div>`
        : '';
    const reasonMarkup = entry.blockedReason
        ? `<div class="mt-3 rounded-2xl border border-rose-400/25 bg-rose-500/10 px-3 py-3 text-sm font-semibold text-rose-100">${escapeHtml(entry.blockedReason)}</div>`
        : '';
    return `
        <div class="rounded-[24px] border border-slate-700/60 bg-slate-900/45 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-400">${escapeHtml(activityLabel)}</div>
                    <div class="mt-2 text-base font-semibold text-slate-50">${escapeHtml(primaryName)}</div>
                    <div class="mt-1 text-sm text-slate-300">${escapeHtml(secondaryText)}</div>
                </div>
                ${renderStatusBadge(entry.status || 'pending')}
            </div>
            <div class="mt-4 grid gap-3 sm:grid-cols-3">
                <div class="rounded-2xl border border-slate-700/60 bg-slate-950/40 px-4 py-3">
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-400">Qualifying Deposit</div>
                    <div class="mt-2 text-sm font-semibold text-slate-50">${escapeHtml(qualifyingDepositAmount ? formatMoneyPrecise(qualifyingDepositAmount) : 'Pending')}</div>
                </div>
                <div class="rounded-2xl border border-slate-700/60 bg-slate-950/40 px-4 py-3">
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-400">Bonus</div>
                    <div class="mt-2 text-sm font-semibold text-slate-50">${escapeHtml(bonusValue ? formatMoneyPrecise(bonusValue) : 'Pending')}</div>
                </div>
                <div class="rounded-2xl border border-slate-700/60 bg-slate-950/40 px-4 py-3">
                    <div class="text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-400">Updated</div>
                    <div class="mt-2 text-sm font-semibold text-slate-50">${escapeHtml(formatRelativeTime(entry.bonusIssuedAt || entry.createdAt))}</div>
                </div>
            </div>
            ${reasonMarkup}
            ${flagsMarkup}
...
function clearReferralProgramView() {
    state.referralProgram = null;
    qs('referral-code-display') && (qs('referral-code-display').textContent = '—');
    qs('referral-link-display') && (qs('referral-link-display').textContent = 'Login required to generate your link.');
    qs('referral-joined-count') && (qs('referral-joined-count').textContent = '0');
    qs('referral-total-earned') && (qs('referral-total-earned').textContent = '0 PKR');
    qs('referral-bonus-status') && (qs('referral-bonus-status').textContent = 'Share your referral link to start earning.');
    if (qs('referral-program-list')) {
        qs('referral-program-list').innerHTML = renderEmptyState('No referrals yet', 'Share your referral link to start earning bonus credits.');
    }
    setReferralProgramMessage('');
}

function renderReferralProgram(summary) {
    if (!summary) {
        clearReferralProgramView();
        return;
    }
    state.referralProgram = summary;
    if (qs('referral-code-display')) {
        qs('referral-code-display').textContent = summary.referralCode || '—';
    }
    if (qs('referral-link-display')) {
        qs('referral-link-display').textContent = summary.referralLink || 'Referral link unavailable';
    }
    if (qs('referral-joined-count')) {
        qs('referral-joined-count').textContent = String(Number(summary.joinedUsers || 0));
    }
    if (qs('referral-total-earned')) {
        qs('referral-total-earned').textContent = formatMoneyPrecise(summary.totalEarnedBonus || 0);
    }

    const referralBonusStatus = qs('referral-bonus-status');
    if (referralBonusStatus) {
        if (summary.incomingReferral?.status === 'blocked') {
            referralBonusStatus.textContent = summary.incomingReferral.blockedReason || 'Bonus blocked by anti-fraud protection.';
        } else if (summary.incomingReferral?.status === 'review') {
            referralBonusStatus.textContent = summary.incomingReferral.blockedReason || 'Referral is under review.';
        } else if (summary.incomingReferral?.bonusIssued) {
            referralBonusStatus.textContent = `Welcome bonus credited: ${formatMoneyPrecise(summary.incomingReferral.referredBonus || 0)}.`;
        } else if (summary.incomingReferral) {
            referralBonusStatus.textContent = `Referral linked with ${summary.incomingReferral.referrerName || 'your referrer'}. Deposit Rs. 170 or more to unlock bonus.`;
        } else {
            referralBonusStatus.textContent = 'Share your referral link to start earning.';
        }
    }

    if (summary.incomingReferral?.status === 'blocked') {
        setReferralProgramMessage(summary.incomingReferral.blockedReason || 'Referral bonus not allowed due to anti-spam protection.', 'error');
    } else if (summary.incomingReferral?.status === 'review') {
        setReferralProgramMessage(summary.incomingReferral.blockedReason || 'Suspicious referral requires admin review.', 'warning');
    } else if (summary.incomingReferral?.bonusIssued) {
        setReferralProgramMessage(`Welcome bonus credited: ${formatMoneyPrecise(summary.incomingReferral.referredBonus || 0)}.`, 'success');
    } else if (summary.incomingReferral) {
        setReferralProgramMessage(`Referral linked with ${summary.incomingReferral.referrerName || 'your referrer'}. Deposit Rs. 170 or more to unlock the welcome bonus.`, 'info');
    } else {
        setReferralProgramMessage('');
    }

    const activityMarkup = [];
    if (summary.incomingReferral) {
        activityMarkup.push(renderReferralActivityCard(summary.incomingReferral, 'incoming'));
    }
    if (Array.isArray(summary.referrals)) {
        summary.referrals.forEach((entry) => {
            activityMarkup.push(renderReferralActivityCard(entry, 'outgoing'));
        });
    }
    qs('referral-program-list').innerHTML = activityMarkup.length
        ? activityMarkup.join('')
        : renderEmptyState('No referrals yet', 'Copy and share your referral link to start tracking joined users and bonus credits.');
}
...
async function loadReferralProgram() {
    ...
    try {
        const summary = await fetchJSON('/api/referral-program');
        renderReferralProgram(summary);
        return summary;
    } catch (err) {
        clearReferralProgramView();
        if (qs('referral-program-list')) {
            qs('referral-program-list').innerHTML = renderEmptyState('Referral program unavailable', err.message || 'Could not load referral information right now.');
        }
        setReferralProgramMessage(err.message || 'Could not load referral information right now.', 'error');
        return null;
    }
}

function openReferralProgram(options = {}) {
    if (!state.currentUser) {
        openAuthModal('login');
        return;
    }
    const modal = qs('referral-program-modal');
    if (!modal) return;
    openModal('referral-program-modal');
    if (!state.referralProgram) {
        void loadReferralProgram();
    }
    if (options.scroll !== false) {
        qs('referral-program-list')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
}

function hideReferralProgram() {
    closeModal('referral-program-modal');
}

async function shareReferralLink() {
    const referralLink = String(state.referralProgram?.referralLink || '').trim();
    if (!referralLink) {
    ...
    if (navigator.share) {
        try {
            await navigator.share({
                title: 'MRF SMS Referral Link',
                text: 'Join MRF SMS using my referral link and unlock bonus credits.',
                url: referralLink
            });
            return;
        } catch {
...
function syncAccountShortcutButtons() {
    ...
    qs('header-referral-button')?.classList.toggle('hidden', !isLoggedIn);
    if (!isLoggedIn) {
        hideAccountDetails();
        hideHeaderQuickMenu();
    }
...
function renderAdminReferrals(referrals) {
    if (!referrals.length) {
        return renderEmptyState('No referral activity yet', 'Joined users, bonus issuance, and blocked suspicious attempts will appear here.');
    }
    const rows = referrals.map((entry) => {
        const fraudFlags = Array.isArray(entry.fraudFlags) && entry.fraudFlags.length ? entry.fraudFlags.join(', ') : '—';
        const bonusText = `${formatMoneyPrecise(entry.referrer_bonus || 0)} / ${formatMoneyPrecise(entry.referred_bonus || 0)}`;
        return `
            <tr class="border-b border-slate-200 align-top last:border-b-0">
                <td class="px-4 py-3">
                    <div class="font-semibold text-slate-900">${escapeHtml(entry.referrer_name || 'Unknown')}</div>
                    <div class="mt-1 break-all text-xs text-slate-500">${escapeHtml(entry.referrer_email || '—')}</div>
                </td>
                <td class="px-4 py-3">
                    <div class="font-semibold text-slate-900">${escapeHtml(entry.referred_name || 'Unknown')}</div>
                    <div class="mt-1 break-all text-xs text-slate-500">${escapeHtml(entry.referred_email || '—')}</div>
                </td>
                <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-900">${escapeHtml(entry.qualifying_deposit_amount ? formatMoneyPrecise(entry.qualifying_deposit_amount) : 'Pending')}</td>
                <td class="px-4 py-3 whitespace-nowrap text-slate-600">${escapeHtml(bonusText)}</td>
                <td class="px-4 py-3">${renderStatusBadge(entry.status || 'pending')}</td>
                <td class="px-4 py-3 text-slate-600">${escapeHtml(entry.blocked_reason || '—')}</td>
                <td class="px-4 py-3 text-slate-600">${escapeHtml(fraudFlags)}</td>
                <td class="px-4 py-3 whitespace-nowrap text-slate-500">${escapeHtml(formatRelativeTime(entry.bonus_issued_at || entry.updated_at || entry.created_at))}</td>
            </tr>
        `;
    }).join('');
    return renderAdminTable(
        ['Referrer', 'Referred User', 'Deposit', 'Bonuses (Referrer / User)', 'Status', 'Blocked Reason', 'Fraud Flags', 'Updated'],
        rows,
        'min-w-[1500px]'
    );
}
...
async function loadAdminData() {
    ...
    try {
        ...
        qs('admin-referrals-list').innerHTML = renderAdminReferrals(referrals);
    ...
    try {
        const response = await fetch('/api/register', {
            ...
            body: JSON.stringify({
                ...
                email,
                password,
                referralCode: state.pendingReferralCode,
...
function showStartupMessages() {
    const referralBootstrap = hydratePendingReferralCode();
    const referralCode = referralBootstrap.code;
    const params = new URLSearchParams(window.location.search);
    const googleError = params.get('google_error');
    const shouldOpenRegister = window.location.pathname === '/signup' || params.get('signup') === '1' || referralBootstrap.fromUrl;
    const shouldOpenLogin = !shouldOpenRegister && params.get('login') === '1';
    const cleanedPathname = window.location.pathname === '/signup' ? '/' : window.location.pathname;
    if (googleError) {
        showToast(`Google sign-in error: ${googleError.replace(/_/g, ' ')}`, 'error', 6000);
        params.delete('google_error');
    }
    if (params.has('signup')) params.delete('signup');
    if (params.has('login')) params.delete('login');
    if (googleError || shouldOpenRegister || shouldOpenLogin) {
        const newUrl = `${cleanedPathname}${params.toString() ? `?${params.toString()}` : ''}`;
        window.history.replaceState({}, document.title, newUrl);
    }
    if (referralBootstrap.fromUrl && referralCode) {
        showToast(`Referral code ${referralCode} applied. Create your account to claim bonus credits.`, 'success', 5000);
    }
...
function bindStaticEvents() {
    ...
    qs('google-login-btn').addEventListener('click', () => {
        ...
        if (state.pendingReferralCode) params.set('ref', state.pendingReferralCode);
        if (state.deviceFingerprint) params.set('deviceFingerprint', state.deviceFingerprint);
        if (state.browserFingerprint) params.set('browserFingerprint', state.browserFingerprint);
        window.location.href = `/auth/google${params.toString() ? `?${params.toString()}` : ''}`;
    });
    ...
    qs('addFundsForm').addEventListener('submit', async (event) => {
        ...
        const senderAccount = String(qs('payment-sender-account-input')?.value || '').trim();
        ...
        if (!senderAccount) {
            showPaymentFormError('Sender Easypaisa account is required');
            return;
        }
        if (!screenshotFile) {
        ...
        try {
            ...
            formData.append('deviceFingerprint', state.deviceFingerprint);
            formData.append('browserFingerprint', state.browserFingerprint);
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (action === 'open-referral-program') {
            hideHeaderQuickMenu();
            openReferralProgram();
            return;
        }
        if (action === 'hide-referral-program') {
            hideReferralProgram();
            return;
        }
