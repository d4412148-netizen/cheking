...
const DEVICE_ID_STORAGE_KEY = 'mrf-device-id-v1';
...
function getOrCreateDeviceId() {
    try {
        ...
        const generated = `dev-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 11)}`;
        window.localStorage.setItem(DEVICE_ID_STORAGE_KEY, generated);
        return generated;
    } catch {
        return `dev-fallback-${Math.random().toString(36).slice(2, 11)}`;
    }
}
...
function getSignupReferralCode() {
    const fieldValue = String(qs('reg-referral-code')?.value || '').trim();
    if (fieldValue) return fieldValue.toUpperCase();
    const params = new URLSearchParams(window.location.search);
    return String(params.get(REFERRAL_QUERY_KEY) || '').trim().toUpperCase();
}

function getReferralLinkForCurrentUser() {
    const code = String(state.currentUser?.referralCode || '').trim();
    if (!code) return '';
    return `mrfsms.com/signup?ref=${encodeURIComponent(code)}`;
}
...
function openReferralModal() {
    ...
    const link = getReferralLinkForCurrentUser();
    const field = qs('referral-link-field');
    const copyBtn = qs('copy-referral-link-btn');
    ...
    try {
        const response = await fetch('/api/register', {
            ...
            body: JSON.stringify({
                ...
                email,
                password,
                referralCode: getSignupReferralCode(),
...
function bindStaticEvents() {
    ...
    qs('google-login-btn').addEventListener('click', () => {
        const referral = getSignupReferralCode();
        window.location.href = referral ? `/auth/google?${REFERRAL_QUERY_KEY}=${encodeURIComponent(referral)}` : '/auth/google';
    });
    const referralFromUrl = String(new URLSearchParams(window.location.search).get(REFERRAL_QUERY_KEY) || '').trim().toUpperCase();
    const referralField = qs('reg-referral-code');
    if (referralField && referralFromUrl && !String(referralField.value || '').trim()) {
        referralField.value = referralFromUrl;
    }
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (action === 'copy-referral-link') {
            const link = qs('referral-link-field')?.value || '';
            if (!link) return;
