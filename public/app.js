...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const orderTime = formatClockTime(order.created_at);
        ...
        const statusRowText = lifecycleStatus === 'pending'
            ? 'Waiting for SMS'
            : lifecycleStatus === 'active'
                ? 'OTP received'
                : lifecycleStatus === 'completed'
        ...
        const footerNotes = lifecycleStatus === 'pending'
            ? [
                ...
                `<span>Expires in <span data-order-expiry-id="${escapeAttr(order.id)}">${escapeHtml(getOrderExpiryText(order))}</span></span>`
            ]
            : lifecycleStatus === 'active'
                ? [`<span>Expires in <span data-order-expiry-id="${escapeAttr(order.id)}">${escapeHtml(getOrderExpiryText(order))}</span></span>`]
        ...
        return `
            <article class="sms-order-card ${isOtpReady ? 'sms-order-card--ready' : lifecycleStatus === 'pending' ? 'sms-order-card--waiting' : 'sms-order-card--closed'}">
                ...
                <div class="sms-order-status-row ${isOtpReady ? 'is-ready' : lifecycleStatus === 'pending' ? '' : 'is-closed'}">
                    ${lifecycleStatus === 'pending'
                        ? '<span class="sms-order-inline-spinner" aria-hidden="true"></span>'
                        : `<span class="sms-order-inline-dot ${isOtpReady ? 'is-ready' : 'is-closed'}" aria-hidden="true"></span>`}
                    <span>${escapeHtml(statusRowText)}</span>
                </div>
                <div class="sms-order-info-row">
                    ...
                    <span>Time: <strong>${escapeHtml(orderTime)}</strong></span>
                    <span class="sms-order-info-divider" aria-hidden="true"></span>
                    <span>Status: <strong>${escapeHtml(statusValue)}</strong></span>
                </div>
                ${contentPanel}
                ${actionButtons ? `<div class="sms-order-actions">${actionButtons}</div>` : ''}
                <div class="sms-order-foot">
                    ${footerNotes.map((note) => `<div class="sms-order-secondary-note">${note}</div>`).join('')}
                </div>
            </article>
        `;
    }).join('');
