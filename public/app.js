...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const statusNote = isOtpReady
            ? 'SMS code received. Tap Finish Activation to close this order.'
        ...
        const timeStatus = getSmsOrderTimeStatus(order, lifecycleStatus);
        const contentPanel = order.otp_code
            ? `
                    <div class="sms-order-otp-box">
                        <div class="sms-order-otp-label">SMS code received</div>
                        <div class="sms-order-otp-value">${escapeHtml(order.otp_code)}</div>
                    </div>
                `
            : isVisibleInWaiting
                ? `
                    <div class="sms-order-live-box">
                        ...
                        <div class="sms-order-live-copy min-w-0">
                            ...
                            <div class="sms-order-live-text">${escapeHtml(waitingDescription)}</div>
                        </div>
                    </div>
                `
                : `
        ...
        const actionButtons = isOtpReady
            ? `
                <button class="sms-order-btn complete" data-action="complete-order" data-order-id="${escapeAttr(order.id)}">Finish Activation</button>
            `
            : lifecycleStatus === 'pending'
                ? `
                    <button class="sms-order-btn cancel" data-action="cancel-order" data-order-id="${escapeAttr(order.id)}" data-cancel-order-id="${escapeAttr(order.id)}" ${canCancel ? '' : 'disabled'}>Cancel</button>
                `
                : '';
        return `
            <article class="sms-order-card ${isOtpReady ? 'sms-order-card--ready' : lifecycleStatus === 'pending' ? 'sms-order-card--waiting' : 'sms-order-card--closed'}">
                <div class="sms-order-head">
                    <div class="sms-order-brand">
                        ${renderServiceLogo(order.service_type, 'md')}
                        <div class="sms-order-brand-copy min-w-0">
                    ...
                    <div class="sms-order-phone-cluster">
                        ...
                        ${order.phone_number ? `<button type="button" class="sms-copy-button" data-action="copy-number" data-value="${escapeAttr(order.phone_number)}" title="Copy number" aria-label="Copy number"><i class="fa-regular fa-copy"></i></button>` : ''}
                    </div>
                </div>
                <div class="sms-order-metrics">
                    <div class="sms-order-metric">
                        <span class="sms-order-metric-label">Status</span>
                        <span class="sms-order-metric-value">${escapeHtml(primaryStatus)}</span>
                    </div>
                    <div class="sms-order-metric">
                        <span class="sms-order-metric-label">Time</span>
                        <span class="sms-order-metric-value" data-order-expiry-id="${escapeAttr(order.id)}">${escapeHtml(timeStatus)}</span>
                    </div>
                    <div class="sms-order-metric">
                        <span class="sms-order-metric-label">Price</span>
                        <span class="sms-order-metric-value">${escapeHtml(formatMoneyPrecise(order.price))}</span>
                    </div>
                </div>
                ${actionButtons ? `<div class="sms-order-actions">${actionButtons}</div>` : ''}
                ${contentPanel}
                <div class="sms-order-foot">
                    ${isWaitingOrder(order) ? `<div class="sms-order-cancel-note">Cancel available in <span data-cancel-unlock-id="${escapeAttr(order.id)}">${escapeHtml(getCancelUnlockText(order))}</span></div>` : ''}
