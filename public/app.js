...
function getServiceLogoApiUrl(serviceType) {
    return `/api/logo/${encodeURIComponent(serviceType)}/file`;
}
...
function renderServiceBrandImageGlyph(serviceType) {
    const meta = getServiceMeta(serviceType);
    const logoUrl = getServiceLogoApiUrl(serviceType);
    const fallbackLogoUrl = LOCAL_DEFAULT_LOGO_URL;
    const secondaryFallbackLogoUrl = meta.logoUrl || serviceBrandLogoUrls[serviceType] || getGeneratedServiceLogoUrl(meta.label || serviceType) || getServiceLogoFallbackUrl(meta.label, serviceType);
    if (!logoUrl) return '';
    return `
        <span class="relative inline-flex h-full w-full items-center justify-center rounded-[7px] bg-white/95 p-[2px]">
            <img src="${escapeAttr(logoUrl)}" data-fallback-src="${escapeAttr(fallbackLogoUrl)}" data-second-fallback-src="${escapeAttr(secondaryFallbackLogoUrl)}" alt="${escapeAttr(meta.label)} logo" loading="lazy" referrerpolicy="no-referrer" class="h-full w-full object-contain rounded-[6px]" onerror="if(this.dataset.fallbackSrc && this.dataset.fallbackApplied !== 'true'){this.dataset.fallbackApplied='true';this.src=this.dataset.fallbackSrc;return;}if(this.dataset.secondFallbackSrc && this.dataset.secondFallbackApplied !== 'true'){this.dataset.secondFallbackApplied='true';this.src=this.dataset.secondFallbackSrc;return;}this.classList.add('hidden');this.nextElementSibling.classList.remove('hidden');">
