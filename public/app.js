...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const statusDetailMarkup = lifecycleStatus === 'pending'
            ? `<span class="sms-order-inline-spinner" aria-hidden="true"></span><strong>${escapeHtml(statusValue)}</strong>`
            : `<span class="sms-order-inline-dot ${isOtpReady ? 'is-ready' : 'is-closed'}" aria-hidden="true"></span><strong>${escapeHtml(statusValue)}</strong>`;
        ...
        return `
            <article class="sms-order-card ${isOtpReady ? 'sms-order-card--ready' : lifecycleStatus === 'pending' ? 'sms-order-card--waiting' : 'sms-order-card--closed'}">
                ...
                <div class="sms-order-info-row">
                    ...
                    <span>Expire Time: ${expireValueMarkup}</span>
                    <span class="sms-order-info-divider" aria-hidden="true"></span>
                ...
                ${footerNotes.length ? `<div class="sms-order-foot">${footerNotes.map((note) => `<div class="sms-order-secondary-note">${note}</div>`).join('')}</div>` : ''}
            </article>
        `;
    }).join('');
    refreshInlineOrderCountdowns();
