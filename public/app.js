...
const REFERRAL_BLOCKED_MESSAGE = 'Referral bonus not allowed (security protection)';
...
function getCountryStatusMeta(country) {
    if (String(state.currentService || '').trim().toLowerCase() !== 'whatsapp') return null;
...
async function loadAdminData() {
    ...
    try {
        ...
        const [orders, paymentRequests, ledgerTransactions, pendingTransactions, users, balanceAdjustments, referralData] = await Promise.all([
        ...
        qs('admin-referral-bonuses-list').innerHTML = renderAdminReferralBonuses(referralData?.bonuses || []);
        qs('admin-referral-fraud-list').innerHTML = renderAdminReferralFraud(referralData?.fraudAttempts || []);
...
async function register(name, email, password) {
    ...
    clearAuthFormError();
    try {
        const registrationMeta = state.registrationMeta || { referralCode: '', browserFingerprint: '', deviceFingerprint: '' };
        const response = await fetch('/api/register', {
            ...
            body: JSON.stringify({
                ...
                email,
                password,
                referralCode: registrationMeta.referralCode || '',
                browserFingerprint: registrationMeta.browserFingerprint || '',
                deviceFingerprint: registrationMeta.deviceFingerprint || ''
            }),
            credentials: 'include'
        });
        if (!response.ok) throw new Error(await response.text());
...
function setPrefilledReferralCode(code) {
    const normalized = String(code || '').trim().toUpperCase();
    const input = qs('reg-referral');
    if (input) {
        input.value = normalized;
    }
}

function buildSimpleFingerprint() {
    const parts = [
        navigator.userAgent || '',
        navigator.platform || '',
        navigator.language || '',
    ...
    return btoa(parts.join('|')).slice(0, 90);
}
...
function hydrateRegistrationMeta() {
    const params = new URLSearchParams(window.location.search);
    const refCode = String(params.get('ref') || '').trim().toUpperCase();
    setPrefilledReferralCode(refCode);
    state.registrationMeta = {
        referralCode: refCode,
        browserFingerprint: buildSimpleFingerprint(),
...
async function loadReferralInfo() {
    ...
    try {
        const info = await fetchJSON('/api/referral/me');
        state.referralInfo = info;
        qs('referral-link-value').textContent = info.referralLink || '—';
        const blockedNotice = qs('referral-security-message');
        if (blockedNotice) {
            blockedNotice.classList.toggle('hidden', Number(info.blockedAttempts || 0) <= 0);
            blockedNotice.textContent = info.blockedMessage || REFERRAL_BLOCKED_MESSAGE;
        }
        return info;
    } catch {
        state.referralInfo = null;
...
function renderAdminReferralBonuses(items) {
    if (!items?.length) {
        return renderEmptyState('No referral bonuses yet', 'When referral rewards are approved, records will appear here.');
    }
    const rows = items.map((item) => `
        <tr class="border-b border-slate-200 align-top last:border-b-0">
            <td class="px-4 py-3 break-all text-slate-700">${escapeHtml(item.referrer_email || '—')}</td>
            <td class="px-4 py-3 break-all text-slate-700">${escapeHtml(item.referred_email || '—')}</td>
            <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-900">${escapeHtml(formatMoneyPrecise(item.deposit_amount))}</td>
            <td class="px-4 py-3 whitespace-nowrap font-semibold text-emerald-700">${escapeHtml(formatMoneyPrecise(item.referrer_bonus))}</td>
            <td class="px-4 py-3 whitespace-nowrap font-semibold text-emerald-700">${escapeHtml(formatMoneyPrecise(item.referred_bonus))}</td>
            <td class="px-4 py-3 break-all text-slate-600">${escapeHtml(item.payment_account || '—')}</td>
            <td class="px-4 py-3 whitespace-nowrap text-slate-500">${escapeHtml(formatRelativeTime(item.created_at))}</td>
        </tr>
    `).join('');
    return renderAdminTable(['Referrer', 'Referred User', 'Deposit', 'Referrer Bonus', 'New User Bonus', 'Payment Account', 'Date'], rows, 'min-w-[1260px]');
}

function renderAdminReferralFraud(items) {
    if (!items?.length) {
        return renderEmptyState('No blocked attempts', 'Fraud checks are active. Blocked attempts will be listed here.');
    }
    const rows = items.map((item) => `
        <tr class="border-b border-slate-200 align-top last:border-b-0">
            <td class="px-4 py-3 break-all text-slate-700">${escapeHtml(item.referrer_email || '—')}</td>
            ...
            <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-900">${escapeHtml(formatMoneyPrecise(item.deposit_amount))}</td>
            <td class="px-4 py-3 break-all text-slate-600">${escapeHtml(item.payment_account || '—')}</td>
            <td class="px-4 py-3 font-semibold text-rose-700">${escapeHtml(item.reason || 'blocked')}</td>
            <td class="px-4 py-3 break-all text-slate-600">${escapeHtml(item.details || '—')}</td>
            <td class="px-4 py-3 whitespace-nowrap text-slate-500">${escapeHtml(formatRelativeTime(item.created_at))}</td>
        </tr>
    `).join('');
    return renderAdminTable(['Referrer', 'Referred User', 'Deposit', 'Payment Account', 'Reason', 'Details', 'Date'], rows, 'min-w-[1260px]');
}
...
function bindStaticEvents() {
    ...
    qs('register-form').addEventListener('submit', async (event) => {
        event.preventDefault();
        const referralCode = String(qs('reg-referral')?.value || '').trim().toUpperCase();
        state.registrationMeta = {
            ...(state.registrationMeta || {}),
            referralCode
        };
    ...
    qs('reg-referral')?.addEventListener('input', (event) => {
        const nextCode = String(event.target?.value || '').replace(/\s+/g, '').toUpperCase();
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (action === 'copy-referral-link') {
            const referralLink = state.referralInfo?.referralLink || qs('referral-link-value')?.textContent || '';
            if (!referralLink || referralLink === '—') return;
            navigator.clipboard.writeText(referralLink).then(() => {
                const copyButton = qs('referral-copy-btn');
