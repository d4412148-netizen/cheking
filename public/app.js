...
async function loadAvailableServices() {
    ...
    try {
        ...
        if (normalizedServices.length) {
            state.availableServices = normalizedServices;
            if (!normalizedServices.some((service) => service.serviceType === state.currentService)) {
                state.currentService = normalizedServices[0].serviceType;
            }
            renderServiceGrid();
            return;
        }
    ...
    try {
        ...
        if (!state.availableServices.some((service) => service.serviceType === state.currentService) && state.availableServices[0]) {
            state.currentService = state.availableServices[0].serviceType;
        }
        renderServiceGrid();
    } catch {
    }
