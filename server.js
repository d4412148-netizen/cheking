...
function normalizeUser(row) {
    ...
    return {
        ...
        referredByUserId: row.referred_by_user_id == null ? null : Number(row.referred_by_user_id),
...
async function evaluateReferralFraudRisk(client, paymentRequest, referredUser) {
    ...
    const referrerRes = await client.query('SELECT id, signup_ip, signup_device_id, signup_browser_fingerprint FROM users WHERE id = $1', [referredUser.referred_by_user_id]);
    ...
    const paymentAccountRef = String(paymentRequest.payment_account_ref || '').trim();
...
app.post('/api/admin/payment-requests/:id/approve', ensureAdmin, async (req, res) => {
    ...
    try {
        ...
        if (Number(paymentRequest.amount || 0) >= 170 && user.referred_by_user_id) {
            const alreadyRewardedRes = await client.query('SELECT id FROM referral_rewards WHERE referred_user_id = $1 LIMIT 1', [user.id]);
            if (alreadyRewardedRes.rows.length === 0) {
                ...
                if (fraudCheck.blocked) {
                    ...
                    await client.query('UPDATE users SET balance = COALESCE(balance, 0) + $1 WHERE id = $2', [50, fraudCheck.referrerUserId]);
                    await client.query(
                        ...
                        [fraudCheck.referrerUserId, user.id, paymentRequest.id, 50, 'awarded']
                    );
                    const referrerEmailRes = await client.query('SELECT email FROM users WHERE id = $1', [fraudCheck.referrerUserId]);
                    await client.query(
                        'INSERT INTO transactions (user_id, user_email, amount, type, status, description, transaction_id, screenshot) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
                        [fraudCheck.referrerUserId, referrerEmailRes.rows[0]?.email || '', 50, 'referral_bonus', 'approved', `Referral bonus from user #${user.id}`, `REF-BONUS-${paymentRequest.id}`, null]
                    );
                }
            }
        }
