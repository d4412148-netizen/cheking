...
const LOGO_CACHE_DIR = path.join(__dirname, 'public', 'logos');
...
const LEGACY_LOGO_CLEANUP_MARKER_PATH = path.join(LOGO_CACHE_DIR, '.legacy-cleanup-complete');
const REMOTE_LOGO_CACHE_SUFFIX = '.cache';
const logoRequestCache = new Map();

function clearLegacyLogoCacheOnce() {
    if (fs.existsSync(LEGACY_LOGO_CLEANUP_MARKER_PATH)) {
        return;
    }
    try {
        const keepFileNames = new Set([path.basename(DEFAULT_LOGO_PUBLIC_PATH).toLowerCase()]);
        const files = fs.readdirSync(LOGO_CACHE_DIR, { withFileTypes: true });
        files.forEach((entry) => {
            if (!entry.isFile()) return;
            const entryName = entry.name.toLowerCase();
            const isRemoteCacheFile = entryName.includes(`${REMOTE_LOGO_CACHE_SUFFIX}.`);
            if (keepFileNames.has(entryName)) {
                return;
            }
            if (entryName.endsWith('.png') && !isRemoteCacheFile) {
                return;
            }
            fs.unlinkSync(path.join(LOGO_CACHE_DIR, entry.name));
        });
        fs.writeFileSync(LEGACY_LOGO_CLEANUP_MARKER_PATH, new Date().toISOString(), 'utf8');
    } catch {
    }
}

clearLegacyLogoCacheOnce();
...
function buildServiceLogoLookupCandidates(value) {
    const raw = String(value || '').trim();
    const normalized = normalizeServiceLookup(raw);
    ...
    normalized.split('/').map((part) => normalizeServiceLookup(part)).filter(Boolean).forEach((part) => {
        candidates.add(part);
        candidates.add(part.replace(/\s+/g, ''));
    });
    return [...candidates].filter(Boolean);
}

function getServiceDomainForLogo(serviceValue) {
    const candidates = buildServiceLogoLookupCandidates(serviceValue);
    for (const candidate of candidates) {
        if (serviceDomains[candidate]) return serviceDomains[candidate];
    }
    return '';
}

function sanitizeLogoCacheName(serviceValue) {
    return String(serviceValue || 'service')
        .toLowerCase()
        .normalize('NFKD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z0-9]+/g, '-')
...
function resolveLocalLogoPublicPath(baseName) {
    const logoFilePath = path.join(LOGO_CACHE_DIR, `${baseName}.png`);
...
function resolveCachedLogoPublicPath(baseName) {
    const supportedExtensions = ['.png', '.jpg', '.jpeg', '.svg', '.webp', '.ico'];
    for (const extension of supportedExtensions) {
        const logoFilePath = path.join(LOGO_CACHE_DIR, `${baseName}${REMOTE_LOGO_CACHE_SUFFIX}${extension}`);
        if (fs.existsSync(logoFilePath)) {
            return `/logos/${baseName}${REMOTE_LOGO_CACHE_SUFFIX}${extension}`;
        }
    }
    return '';
}

function getLogoMissMarkerPath(baseName) {
    return path.join(LOGO_CACHE_DIR, `${baseName}.miss`);
}

function getLogoFileExtension(response, sourceUrl) {
...
function writeLogoMissMarker(baseName, serviceLabel) {
    try {
        fs.writeFileSync(getLogoMissMarkerPath(baseName), JSON.stringify({ service: serviceLabel, createdAt: new Date().toISOString() }), 'utf8');
    } catch {
    }
}

async function downloadRemoteLogo(remoteUrl, baseName) {
    ...
    if (!contentType.startsWith('image/')) {
        throw new Error(`Unexpected logo response type: ${contentType || 'unknown'}`);
    }
    const extension = getLogoFileExtension(response, remoteUrl);
    const targetPath = path.join(LOGO_CACHE_DIR, `${baseName}${REMOTE_LOGO_CACHE_SUFFIX}${extension}`);
...
async function getLogo(serviceValue) {
    const normalizedService = String(serviceValue || '').trim();
    const serviceConfig = getServiceConfig(normalizedService);
    const serviceLabel = getServiceCatalogLabel(serviceConfig) || decodeURIComponent(normalizedService || 'service');
    const cacheBaseName = sanitizeLogoCacheName(serviceConfig?.serviceType || serviceLabel);
    const localPublicPath = resolveLocalLogoPublicPath(cacheBaseName);
    if (localPublicPath) {
        return localPublicPath;
    }
    const cachedPublicPath = resolveCachedLogoPublicPath(cacheBaseName);
    if (cachedPublicPath) {
        return cachedPublicPath;
    }
    if (fs.existsSync(getLogoMissMarkerPath(cacheBaseName))) {
        return DEFAULT_LOGO_PUBLIC_PATH;
    }
    if (logoRequestCache.has(cacheBaseName)) {
        return logoRequestCache.get(cacheBaseName);
    }
    const logoPromise = (async () => {
        const domain = getServiceDomainForLogo(serviceConfig?.serviceType || serviceLabel);
        if (!domain) {
            writeLogoMissMarker(cacheBaseName, serviceLabel);
            return DEFAULT_LOGO_PUBLIC_PATH;
        }
        const remoteUrl = `https://logo.clearbit.com/${domain}`;
        try {
            return await downloadRemoteLogo(remoteUrl, cacheBaseName);
        } catch {
            writeLogoMissMarker(cacheBaseName, serviceLabel);
            return DEFAULT_LOGO_PUBLIC_PATH;
        }
    })();
    logoRequestCache.set(cacheBaseName, logoPromise);
    try {
        return await logoPromise;
    } finally {
        logoRequestCache.delete(cacheBaseName);
    }
}
...
app.get('/api/logo/:service', async (req, res) => {
    try {
        const logoPath = await getLogo(req.params.service);
        res.json({ logo: logoPath });
    } catch {
        res.json({ logo: DEFAULT_LOGO_PUBLIC_PATH });
    }
});

app.get('/api/logo/:service/file', async (req, res) => {
    try {
        const logoPath = await getLogo(req.params.service);
        res.redirect(logoPath);
    } catch {
        res.redirect(DEFAULT_LOGO_PUBLIC_PATH);
    }
});
