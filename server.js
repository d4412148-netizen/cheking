...
function getFirstConfiguredEnvValue(keys, fallback = '') {
...
const BASE_URL = String(getFirstConfiguredEnvValue(['BASE_URL', 'APP_BASE_URL', 'PUBLIC_BASE_URL', 'SITE_URL'], '')).trim().replace(/\/+$/, '');
const GOOGLE_CALLBACK_URL = process.env.GOOGLE_CALLBACK_URL || (BASE_URL ? `${BASE_URL}/auth/google/callback` : '/auth/google/callback');
const APP_BASE_URL = BASE_URL;

function normalizeSmtpPassword(value) {
    const raw = String(value || '');
    const compact = raw.replace(/\s+/g, '');
    return /\s/.test(raw) && compact.length === 16 ? compact : raw;
}

const DEFAULT_SMTP_HOST = 'smtp.gmail.com';
const SMTP_URL = getFirstConfiguredEnvValue(['SMTP_URL', 'EMAIL_URL', 'MAIL_URL']);
const SMTP_SERVICE = getFirstConfiguredEnvValue(['SMTP_SERVICE', 'EMAIL_SERVICE', 'MAIL_SERVICE']);
const CONFIGURED_SMTP_HOST = getFirstConfiguredEnvValue(['SMTP_HOST', 'MAIL_HOST', 'EMAIL_HOST']);
const SMTP_HOST = CONFIGURED_SMTP_HOST || DEFAULT_SMTP_HOST;
const SMTP_PORT = Number(getFirstConfiguredEnvValue(['SMTP_PORT', 'MAIL_PORT', 'EMAIL_PORT'], '587'));
const SMTP_SECURE = String(getFirstConfiguredEnvValue(['SMTP_SECURE', 'MAIL_SECURE', 'EMAIL_SECURE'], '')).toLowerCase() === 'true' || SMTP_PORT === 465;
const SMTP_REQUIRE_TLS = String(getFirstConfiguredEnvValue(['REQUIRE_TLS', 'SMTP_REQUIRE_TLS', 'MAIL_REQUIRE_TLS', 'EMAIL_REQUIRE_TLS'], SMTP_SECURE ? 'false' : 'true')).toLowerCase() === 'true';
const SMTP_USER = getFirstConfiguredEnvValue(['SMTP_USER', 'MAIL_USER', 'EMAIL_USER', 'GMAIL_USER']);
const SMTP_PASS = normalizeSmtpPassword(getFirstConfiguredEnvValue(['SMTP_PASS', 'MAIL_PASS', 'EMAIL_PASS', 'GMAIL_APP_PASSWORD', 'GMAIL_PASS']));
const SMTP_FROM = getFirstConfiguredEnvValue(['SMTP_FROM', 'MAIL_FROM', 'EMAIL_FROM'], SMTP_USER);
const ICONS_DIR = path.join(__dirname, 'public', 'icons');
const DEFAULT_SERVICE_ICON_FILE = path.join(ICONS_DIR, 'default.svg');
const SIMPLE_ICONS_ENDPOINT = 'https://cdn.simpleicons.org';
const ICON_FETCH_TIMEOUT_MS = 12000;
const SERVICE_ICON_SLUG_OVERRIDES = Object.freeze({
    amazon: ['amazon'],
    alibaba: ['alibabadotcom', 'alibaba'],
    aliexpress: ['aliexpress'],
    airbnb: ['airbnb'],
    amazonprime: ['primevideo', 'amazon'],
    apple: ['apple'],
    binance: ['binance'],
    careem: ['careem'],
...
const SERVICE_ICON_PLACEHOLDER_BACKGROUNDS = Object.freeze([
    ['#0f172a', '#1d4ed8'],
    ['#134e4a', '#0f766e'],
    ['#4c1d95', '#7c3aed'],
    ['#7f1d1d', '#dc2626'],
...
const serviceIconCachePromises = new Map();

try {
    fs.mkdirSync(ICONS_DIR, { recursive: true });
} catch (err) {
    console.error(`Service icon directory initialization failed: ${formatSafeError(err, 'Could not prepare icon directory')}`);
}
...
const REFERRER_BONUS_PKR = 50;
const REFERRED_WELCOME_BONUS_PKR = 20;
const REFERRAL_BLOCKED_MESSAGE = 'Referral bonus not allowed due to anti-spam protection.';
const REFERRAL_REVIEW_MESSAGE = 'Suspicious referral requires admin review.';
...
function formatSmtpDebugError(err) {
    if (!err) return 'Unknown SMTP error';
    const details = [
        err.code ? `code=${err.code}` : '',
        err.command ? `command=${err.command}` : '',
        err.responseCode ? `responseCode=${err.responseCode}` : '',
        err.response ? `response=${String(err.response).trim()}` : '',
        err.message ? `message=${String(err.message).trim()}` : ''
    ].filter(Boolean);
    return details.join(' | ') || formatSafeError(err, 'Unknown SMTP error');
}
...
function buildServiceIconKey(value) {
    return normalizeServiceIconLookup(value).replace(/\s+/g, '');
}
...
function buildServiceIconMonogram(value) {
...
function buildGeneratedServiceIconSvg(serviceName, iconKey = '') {
    const normalizedKey = String(iconKey || buildServiceIconKey(serviceName) || 'service').trim() || 'service';
    const paletteIndex = parseInt(crypto.createHash('md5').update(normalizedKey).digest('hex').slice(0, 2), 16) % SERVICE_ICON_PLACEHOLDER_BACKGROUNDS.length;
    const [startColor, endColor] = SERVICE_ICON_PLACEHOLDER_BACKGROUNDS[paletteIndex];
    const label = String(serviceName || 'Service').trim() || 'Service';
    const monogram = buildServiceIconMonogram(label);
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-hidden="true" role="img">
  ...
  <rect width="64" height="64" rx="18" fill="url(#iconGradient)" />
  <circle cx="32" cy="32" r="30" fill="none" stroke="rgba(255,255,255,0.18)" stroke-width="2" />
  <text x="32" y="37" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="22" font-weight="800" letter-spacing="1.5" fill="#ffffff">${escapeXml(monogram)}</text>
</svg>`;
}

function sanitizeRemoteServiceIconSvg(svgMarkup) {
    let normalizedSvg = String(svgMarkup || '').trim();
    if (!normalizedSvg.toLowerCase().startsWith('<svg')) {
        return '';
    }
    normalizedSvg = normalizedSvg.replace(/<\?xml[\s\S]*?\?>/gi, '').trim();
    if (!/xmlns=/.test(normalizedSvg)) {
        normalizedSvg = normalizedSvg.replace('<svg', '<svg xmlns="http://www.w3.org/2000/svg"');
    }
    if (!/aria-hidden=/.test(normalizedSvg)) {
        normalizedSvg = normalizedSvg.replace('<svg', '<svg aria-hidden="true"');
    }
    return normalizedSvg;
}

function buildServiceIconSlugCandidates(serviceName) {
    const normalizedName = normalizeServiceIconLookup(serviceName);
    const compactName = normalizedName.replace(/\s+/g, '');
    const candidates = [];
    const seen = new Set();
    const addCandidate = (value) => {
        const slug = String(value || '').toLowerCase().replace(/[^a-z0-9]+/g, '').trim();
        if (!slug || seen.has(slug)) return;
        seen.add(slug);
        candidates.push(slug);
    };
    const explicitCandidates = SERVICE_ICON_SLUG_OVERRIDES[compactName] || [];
    explicitCandidates.forEach(addCandidate);
    addCandidate(compactName);
    normalizedName.split(/\s+/).forEach(addCandidate);
    if (compactName.endsWith('app')) {
        addCandidate(compactName.slice(0, -3));
    }
    if (compactName.endsWith('pay')) {
        addCandidate(compactName.slice(0, -3));
    }
    return candidates;
}

async function fetchRemoteServiceIconSvg(slug) {
    if (!slug || !axios || typeof axios.get !== 'function') {
        return '';
    }
    const response = await axios.get(`${SIMPLE_ICONS_ENDPOINT}/${encodeURIComponent(slug)}`, {
        responseType: 'text',
    ...
    return sanitizeRemoteServiceIconSvg(response.data);
}

async function ensureDefaultServiceIconFile() {
    if (!fs.existsSync(DEFAULT_SERVICE_ICON_FILE)) {
        await fs.promises.mkdir(ICONS_DIR, { recursive: true });
        await fs.promises.writeFile(DEFAULT_SERVICE_ICON_FILE, buildGeneratedServiceIconSvg('Service', 'default'), 'utf8');
    }
    return DEFAULT_SERVICE_ICON_FILE;
}

async function ensureServiceIconFile(serviceName) {
    const iconKey = buildServiceIconKey(serviceName);
    if (!iconKey) {
        return ensureDefaultServiceIconFile();
    }
    const cachedIconPath = path.join(ICONS_DIR, `${iconKey}.svg`);
    if (fs.existsSync(cachedIconPath)) {
        return cachedIconPath;
    }
    if (!serviceIconCachePromises.has(iconKey)) {
        serviceIconCachePromises.set(iconKey, (async () => {
            await fs.promises.mkdir(ICONS_DIR, { recursive: true });
            for (const slug of buildServiceIconSlugCandidates(serviceName)) {
                try {
                    const remoteSvg = await fetchRemoteServiceIconSvg(slug);
                    if (!remoteSvg) continue;
                    await fs.promises.writeFile(cachedIconPath, remoteSvg, 'utf8');
                    return cachedIconPath;
                } catch (err) {
                    console.error(`[ICON][fetch:failed] ${iconKey} <- ${slug} | ${formatSafeError(err, 'Icon fetch failed')}`);
                }
            }
            await fs.promises.writeFile(cachedIconPath, buildGeneratedServiceIconSvg(serviceName, iconKey), 'utf8');
            return cachedIconPath;
        })().finally(() => {
            serviceIconCachePromises.delete(iconKey);
        }));
    }
    return serviceIconCachePromises.get(iconKey);
}

function buildMailTransportCandidates() {
    if ((!SMTP_URL && !SMTP_SERVICE && !CONFIGURED_SMTP_HOST && !(SMTP_USER && SMTP_PASS)) || !SMTP_FROM) {
        return [];
    }
    const inferredSmtpService = !SMTP_URL && !SMTP_SERVICE && !CONFIGURED_SMTP_HOST && SMTP_USER && SMTP_PASS ? 'gmail' : '';
    const normalizedService = String(SMTP_SERVICE || inferredSmtpService || '').trim().toLowerCase();
    const normalizedHost = String(SMTP_HOST || DEFAULT_SMTP_HOST).trim().toLowerCase();
    const baseConfig = {
    ...
    if ((normalizedService === 'gmail' || normalizedHost === 'smtp.gmail.com') && SMTP_USER && SMTP_PASS) {
        return [
            {
                id: 'gmail-tls-587',
                label: 'smtp.gmail.com:587 (TLS)',
                config: {
                    host: 'smtp.gmail.com',
                    port: 587,
                    secure: false,
    ...
    return [{
        id: 'smtp-default',
        label: normalizedService ? `${normalizedService} SMTP` : `${SMTP_HOST}:${SMTP_PORT}`,
        config: {
            ...(normalizedService ? { service: normalizedService } : {}),
            ...(!normalizedService && SMTP_HOST ? {
                host: SMTP_HOST,
                port: SMTP_PORT,
                secure: SMTP_SECURE,
                requireTLS: SMTP_REQUIRE_TLS
...
function createMailTransporters() {
    if (!nodemailer || typeof nodemailer.createTransport !== 'function') {
        console.error('Email transport disabled: nodemailer is not available.');
        return [];
    }
    try {
        return buildMailTransportCandidates().map((candidate) => ({
            ...candidate,
            transporter: nodemailer.createTransport(candidate.config),
            readyPromise: null,
            verified: false
        }));
    } catch (err) {
...
const mailTransporters = createMailTransporters();
let activeMailTransporterId = '';

function getOrderedMailTransporters(preferredId = '') {
    const ordered = [];
    const seen = new Set();
    [preferredId, activeMailTransporterId].filter(Boolean).forEach((id) => {
        const candidate = mailTransporters.find((entry) => entry.id === id);
        if (candidate && !seen.has(candidate.id)) {
            seen.add(candidate.id);
            ordered.push(candidate);
        }
    });
    mailTransporters.forEach((candidate) => {
...
function clearMailTransporterState(candidate) {
    if (!candidate) return;
    candidate.readyPromise = null;
    candidate.verified = false;
    if (activeMailTransporterId === candidate.id) {
        activeMailTransporterId = '';
    }
}

async function verifyMailTransporter(candidate) {
    if (!candidate) return null;
    if (candidate.verified) return candidate;
    if (typeof candidate.transporter.verify !== 'function') {
        candidate.verified = true;
        activeMailTransporterId = candidate.id;
        console.info(`[MAIL][verify:skip] ${candidate.label}`);
        return candidate;
    }
    if (!candidate.readyPromise) {
        console.info(`[MAIL][verify:start] ${candidate.label}`);
        candidate.readyPromise = candidate.transporter.verify()
            .then(() => {
                candidate.verified = true;
                activeMailTransporterId = candidate.id;
                console.info(`[MAIL][verify:success] ${candidate.label}`);
                return candidate;
            })
            .catch((err) => {
                clearMailTransporterState(candidate);
                console.error(`[MAIL][verify:failed] ${candidate.label} | ${formatSmtpDebugError(err)}`);
                throw err;
            });
    }
    return candidate.readyPromise;
}
...
function syncPendingReferralCodeOnSession(req) {
...
app.get(['/', '/index.html', '/dashboard', '/signup'], (req, res) => {
    syncPendingReferralCodeOnSession(req);
...
app.get(['/reset-password', '/reset-password.html'], (_req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'reset-password.html'));
});

app.get('/api/icons/:service', async (req, res) => {
    try {
        const serviceName = decodeURIComponent(String(req.params.service || '')).trim();
        const iconFilePath = await ensureServiceIconFile(serviceName);
        res.type('image/svg+xml');
        res.sendFile(iconFilePath);
    } catch (err) {
        console.error(`[ICON][route:failed] ${formatSafeError(err, 'Could not load service icon')}`);
        try {
            const fallbackIconPath = await ensureDefaultServiceIconFile();
            res.type('image/svg+xml');
            res.sendFile(fallbackIconPath);
        } catch (fallbackErr) {
            res.status(500).send(formatSafeError(fallbackErr, 'Could not load service icon'));
        }
    }
});
...
function normalizeUser(row) {
    ...
    return {
        ...
        referred_by_user_id: row.referred_by_user_id == null ? null : Number(row.referred_by_user_id),
...
function normalizeSenderAccount(value) {
    const normalized = String(value || '').replace(/[^0-9A-Za-z+]/g, '').trim();
...
function getRequestIp(req) {
    ...
    return (forwardedFor || fallbackIp || '').slice(0, 128);
}

function getRequestFingerprintContext(req) {
    return {
        requestIp: getRequestIp(req),
        deviceFingerprint: normalizeFingerprintValue(req?.body?.deviceFingerprint || req?.headers?.['x-device-fingerprint']),
        browserFingerprint: normalizeFingerprintValue(req?.body?.browserFingerprint || req?.headers?.['x-browser-fingerprint'])
    };
}
...
async function generateUniqueReferralCode() {
    for (let attempt = 0; attempt < 25; attempt += 1) {
        ...
        const existing = await queryOne('SELECT id FROM users WHERE referral_code = $1', [referralCode]);
        if (!existing) {
            return referralCode;
        }
    }
...
async function evaluateReferralSignupStatus(client, { referrerUserId, referredUserId, signupIp, deviceFingerprint, browserFingerprint }) {
    const fraudFlags = [];
    if (!referrerUserId || !referredUserId) {
        return { status: 'pending', blockedReason: null, fraudFlags };
    }
    if (Number(referrerUserId) === Number(referredUserId)) {
        fraudFlags.push('self_signup');
        return {
            status: 'blocked',
            blockedReason: REFERRAL_BLOCKED_MESSAGE,
            fraudFlags
        };
    }

    const referrerRes = await client.query('SELECT id, signup_ip, device_fingerprint, browser_fingerprint FROM users WHERE id = $1', [referrerUserId]);
    const referrer = referrerRes.rows[0];
    if (!referrer) {
        return { status: 'blocked', blockedReason: REFERRAL_BLOCKED_MESSAGE, fraudFlags: ['missing_referrer'] };
    }

    if (signupIp && referrer.signup_ip && signupIp === referrer.signup_ip) {
    ...
    if (browserFingerprint && referrer.browser_fingerprint && browserFingerprint === referrer.browser_fingerprint) {
        fraudFlags.push('same_browser');
    }

    const matchingPatternRes = await client.query(`
        SELECT COUNT(*)::int AS total
        FROM referrals
        WHERE referrer_user_id = $1
          AND (
              ($2 <> '' AND signup_ip = $2)
              OR ($3 <> '' AND device_fingerprint = $3)
              OR ($4 <> '' AND browser_fingerprint = $4)
          )
    `, [referrerUserId, signupIp || '', deviceFingerprint || '', browserFingerprint || '']);
    if (Number(matchingPatternRes.rows[0]?.total || 0) >= 1) {
        fraudFlags.push('multiple_fake_accounts');
    }

    if (fraudFlags.some((flag) => ['self_signup', 'same_ip', 'same_device', 'same_browser'].includes(flag))) {
        return {
            status: 'blocked',
            blockedReason: REFERRAL_BLOCKED_MESSAGE,
            fraudFlags
    ...
    if (fraudFlags.includes('multiple_fake_accounts')) {
        return {
            status: 'review',
            blockedReason: REFERRAL_REVIEW_MESSAGE,
            fraudFlags
        };
    }
...
async function createReferralRecord(client, { referrerUserId, referredUserId, referralCode, signupIp, deviceFingerprint, browserFingerprint }) {
    if (!referrerUserId || !referredUserId || !referralCode) return null;
    const evaluation = await evaluateReferralSignupStatus(client, {
        referrerUserId,
        referredUserId,
        signupIp,
        deviceFingerprint,
    ...
    `, [
        ...
        evaluation.fraudFlags.length ? evaluation.fraudFlags.join(', ') : null
    ]);
    return evaluation;
}
...
function isMailConfigured() {
    return mailTransporters.some((candidate) => candidate && candidate.transporter && typeof candidate.transporter.sendMail === 'function');
}
...
function getRequestBaseUrl(req) {
    ...
    const protocol = forwardedProto || req.protocol || (IS_PROD ? 'https' : 'http');
    if (!host) {
        return APP_BASE_URL;
    }
...
function buildAbsoluteUrl(relativePath, baseUrl = APP_BASE_URL) {
...
async function sendPasswordResetEmail(user, token, req) {
    if (!mailTransporters.length) {
        throw new Error('Password reset email is not configured');
    }
    const resetUrl = buildAbsoluteUrl(`/reset-password?token=${encodeURIComponent(token)}`, getRequestBaseUrl(req));
    ...
    for (const candidate of getOrderedMailTransporters()) {
        try {
            await verifyMailTransporter(candidate);
        } catch (err) {
            deliveryErrors.push(`${candidate.label}: ${formatSmtpDebugError(err)}`);
            continue;
        }
        console.info(`[MAIL][send:start] ${candidate.label} -> ${user.email}`);
        try {
            const info = await candidate.transporter.sendMail(mailOptions);
            candidate.verified = true;
            activeMailTransporterId = candidate.id;
            const accepted = Array.isArray(info?.accepted) ? info.accepted.join(',') : '';
            console.info(`[MAIL][send:success] ${candidate.label} -> ${user.email} | messageId=${info?.messageId || 'n/a'}${accepted ? ` | accepted=${accepted}` : ''}`);
            return info;
        } catch (err) {
            clearMailTransporterState(candidate);
            const reason = formatSmtpDebugError(err);
            console.error(`[MAIL][send:failed] ${candidate.label} -> ${user.email} | ${reason}`);
            deliveryErrors.push(`${candidate.label}: ${reason}`);
        }
    }
    const combinedError = deliveryErrors.join(' || ');
    console.error(`[MAIL][all-attempts-failed] ${user.email} | ${combinedError || 'Email delivery failed'}`);
    throw new Error(combinedError || 'Email delivery failed');
}
...
async function initDB() {
    ...
    await queryRun('ALTER TABLE users ADD COLUMN IF NOT EXISTS referred_by_user_id INTEGER');
    ...
    await queryRun('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS sender_account TEXT');
    ...
    await queryRun('ALTER TABLE payment_requests ADD COLUMN IF NOT EXISTS sender_account TEXT');
    ...
    await queryRun(`
        CREATE TABLE IF NOT EXISTS referrals (
            ...
            blocked_reason TEXT,
            fraud_flags TEXT,
            qualifying_deposit_amount NUMERIC(12,2),
            qualifying_transaction_id INTEGER,
            qualifying_payment_request_id INTEGER,
            referrer_bonus NUMERIC(12,2) DEFAULT 0,
            referred_bonus NUMERIC(12,2) DEFAULT 0,
            bonus_issued BOOLEAN DEFAULT FALSE,
            bonus_issued_at TIMESTAMPTZ,
    ...
    await queryRun('CREATE INDEX IF NOT EXISTS idx_referrals_status ON referrals (status, created_at DESC)');
...
END $$;
    ...
    const duplicateReferralCodeRows = await queryAll(`
        SELECT referral_code, ARRAY_AGG(id ORDER BY id) AS user_ids
        FROM users
        WHERE NULLIF(TRIM(COALESCE(referral_code, '')), '') IS NOT NULL
        GROUP BY referral_code
        HAVING COUNT(*) > 1
    `);
    for (const row of duplicateReferralCodeRows) {
        const duplicateUserIds = Array.isArray(row.user_ids) ? row.user_ids.map((value) => Number(value)).filter(Number.isFinite) : [];
        for (const userId of duplicateUserIds.slice(1)) {
            await queryRun('UPDATE users SET referral_code = $1 WHERE id = $2', [await generateUniqueReferralCode(), userId]);
        }
    }

    const usersMissingReferralCodes = await queryAll(`
        SELECT id
        FROM users
        WHERE NULLIF(TRIM(COALESCE(referral_code, '')), '') IS NULL
    `);
    for (const row of usersMissingReferralCodes) {
        await queryRun('UPDATE users SET referral_code = $1 WHERE id = $2', [await generateUniqueReferralCode(), Number(row.id)]);
    }
...
async function createUser(name, email, password, options = {}) {
    const pendingReferralCode = normalizeReferralCode(options.referralCode);
    const signupIp = String(options.signupIp || '').trim().slice(0, 128);
    const deviceFingerprint = normalizeFingerprintValue(options.deviceFingerprint);
    const browserFingerprint = normalizeFingerprintValue(options.browserFingerprint);
    const normalizedName = String(name || '').trim();
    const normalizedEmail = sanitizeEmail(email);
    const referralCode = await generateUniqueReferralCode();
    const hashedPassword = await hashPassword(password);
    const referrer = pendingReferralCode ? await findUserByReferralCode(pendingReferralCode) : null;
    ...
    try {
        ...
        if (referrer) {
            await createReferralRecord(client, {
...
async function approveTransaction(txId) {
    ...
    try {
        ...
        await processReferralBonusForDeposit(client, {
            userId: tx.user_id,
            amount: tx.amount,
            senderAccount: tx.sender_account,
            requestIp: tx.request_ip,
            deviceFingerprint: tx.device_fingerprint,
            browserFingerprint: tx.browser_fingerprint,
            transactionId: tx.id,
...
async function getUserCombinedHistory(userId, limit = 300) {
    const rows = await queryAll(`
        ...
        FROM (
            ...
            FROM transactions t
            WHERE t.user_id = $1
              AND LOWER(COALESCE(t.type, '')) IN ('deposit', 'referral_referrer_bonus', 'referral_welcome_bonus')
...
async function getReferralProgramSummary(userId, baseUrl = APP_BASE_URL) {
    const user = await findUserById(userId);
    if (!user) {
        throw new Error('User not found');
    }
    const referralRows = await queryAll(`
        SELECT
            r.*,
    ...
    `, [userId]);
    const incomingReferral = await queryOne(`
        SELECT
            r.*,
    ...
    return {
        ...
        referralLink: buildAbsoluteUrl(`/signup?ref=${encodeURIComponent(user.referralCode)}`, baseUrl),
        joinedUsers: referralRows.length,
        totalEarnedBonus: referralRows.reduce((sum, row) => sum + Number(row.bonus_issued ? row.referrer_bonus || 0 : 0), 0),
        referrals: referralRows.map((row) => ({
            id: row.id,
            name: row.referred_name || 'Friend',
            email: row.referred_email || 'Unknown email',
            status: String(row.status || 'pending'),
            blockedReason: row.blocked_reason || '',
            qualifyingDepositAmount: Number(row.qualifying_deposit_amount || 0),
            referrerBonus: Number(row.referrer_bonus || 0),
            referredBonus: Number(row.referred_bonus || 0),
            bonusIssued: Boolean(row.bonus_issued),
            bonusIssuedAt: row.bonus_issued_at,
            fraudFlags: parseFraudFlags(row.fraud_flags),
            createdAt: row.created_at
        })),
        incomingReferral: incomingReferral ? {
            id: incomingReferral.id,
            status: String(incomingReferral.status || 'pending'),
            blockedReason: incomingReferral.blocked_reason || '',
            referrerName: incomingReferral.referrer_name || 'Referral Partner',
            referrerEmail: incomingReferral.referrer_email || '',
            qualifyingDepositAmount: Number(incomingReferral.qualifying_deposit_amount || 0),
            referrerBonus: Number(incomingReferral.referrer_bonus || 0),
            referredBonus: Number(incomingReferral.referred_bonus || 0),
            bonusIssued: Boolean(incomingReferral.bonus_issued),
            fraudFlags: parseFraudFlags(incomingReferral.fraud_flags),
            createdAt: incomingReferral.created_at
        } : null
    };
}

async function getAdminReferrals(limit = 300) {
    const rows = await queryAll(`
        SELECT
            r.*,
            referrer.name AS referrer_name,
    ...
    return rows.map((row) => ({
        ...row,
        qualifying_deposit_amount: Number(row.qualifying_deposit_amount || 0),
        referrer_bonus: Number(row.referrer_bonus || 0),
        referred_bonus: Number(row.referred_bonus || 0),
        bonus_issued: Boolean(row.bonus_issued),
        fraudFlags: parseFraudFlags(row.fraud_flags)
    }));
}

async function processReferralBonusForDeposit(client, {
    userId,
    amount,
    senderAccount,
    requestIp,
    ...
    if (!Number.isFinite(depositAmount) || depositAmount < REFERRAL_MIN_DEPOSIT_PKR) {
        return { applied: false, status: 'below_minimum' };
    }
    ...
    const referralRes = await client.query(`
        SELECT
            ...
            referrer.email AS referrer_email,
            referrer.name AS referrer_name,
            referrer.signup_ip AS referrer_signup_ip,
            referrer.device_fingerprint AS referrer_device_fingerprint,
            referrer.browser_fingerprint AS referrer_browser_fingerprint,
            referred.email AS referred_email,
            referred.name AS referred_name
        FROM referrals r
    ...
    if (!referral) {
        return { applied: false, status: 'no_referral' };
    }
    if (referral.bonus_issued) {
        return { applied: false, status: 'already_issued' };
    }

    const fraudFlags = new Set(parseFraudFlags(referral.fraud_flags));
    const normalizedSenderAccount = normalizeSenderAccount(senderAccount);
    const effectiveIp = String(requestIp || referral.signup_ip || '').trim();
    const effectiveDeviceFingerprint = normalizeFingerprintValue(deviceFingerprint || referral.device_fingerprint);
    const effectiveBrowserFingerprint = normalizeFingerprintValue(browserFingerprint || referral.browser_fingerprint);

    if (Number(referral.referrer_user_id) === Number(referral.referred_user_id)) {
        fraudFlags.add('self_signup');
    }
    if (effectiveIp && referral.referrer_signup_ip && effectiveIp === referral.referrer_signup_ip) {
        fraudFlags.add('same_ip');
    }
    if (effectiveDeviceFingerprint && referral.referrer_device_fingerprint && effectiveDeviceFingerprint === referral.referrer_device_fingerprint) {
        fraudFlags.add('same_device');
    }
    if (effectiveBrowserFingerprint && referral.referrer_browser_fingerprint && effectiveBrowserFingerprint === referral.referrer_browser_fingerprint) {
        fraudFlags.add('same_browser');
    }
    if (normalizedSenderAccount) {
        const repeatedSenderRes = await client.query(`
            SELECT COUNT(*)::int AS total
            FROM transactions
            WHERE sender_account = $1
              AND user_id <> $2
              AND LOWER(COALESCE(status, '')) = 'approved'
        `, [normalizedSenderAccount, userId]);
        if (Number(repeatedSenderRes.rows[0]?.total || 0) > 0) {
            fraudFlags.add('repeated_easypaisa_account');
        }
    }

    const suspiciousPatternRes = await client.query(`
        SELECT COUNT(*)::int AS total
        FROM referrals
        WHERE referrer_user_id = $1
          ...
          AND (
              ($3 <> '' AND signup_ip = $3)
              OR ($4 <> '' AND device_fingerprint = $4)
              OR ($5 <> '' AND browser_fingerprint = $5)
          )
    `, [
        ...
        effectiveBrowserFingerprint || ''
    ]);
    if (Number(suspiciousPatternRes.rows[0]?.total || 0) >= 1) {
        fraudFlags.add('multiple_fake_accounts');
    }

    const fraudFlagsList = [...fraudFlags];
    const blockedFlags = ['self_signup', 'same_ip', 'same_device', 'same_browser', 'repeated_easypaisa_account'];
    if (fraudFlagsList.some((flag) => blockedFlags.includes(flag))) {
        await client.query(`
            UPDATE referrals
            SET status = 'blocked',
                blocked_reason = $1,
                fraud_flags = $2,
                qualifying_deposit_amount = $3,
                qualifying_transaction_id = $4,
                qualifying_payment_request_id = $5,
                updated_at = CURRENT_TIMESTAMP
            WHERE id = $6
        `, [
            REFERRAL_BLOCKED_MESSAGE,
            fraudFlagsList.join(', '),
            depositAmount,
            transactionId || null,
            paymentRequestId || null,
        ...
        return { applied: false, status: 'blocked', message: REFERRAL_BLOCKED_MESSAGE };
    }

    if (fraudFlags.has('multiple_fake_accounts')) {
        await client.query(`
            UPDATE referrals
            SET status = 'review',
                blocked_reason = $1,
        ...
        return { applied: false, status: 'review', message: REFERRAL_REVIEW_MESSAGE };
    }

    await client.query('UPDATE users SET balance = COALESCE(balance, 0) + $1 WHERE id = $2', [REFERRER_BONUS_PKR, referral.referrer_user_id]);
    await client.query('UPDATE users SET balance = COALESCE(balance, 0) + $1 WHERE id = $2', [REFERRED_WELCOME_BONUS_PKR, userId]);

    await client.query(
        'INSERT INTO balance_adjustments (user_id, amount, reason, admin_id) VALUES ($1, $2, $3, $4)',
        [referral.referrer_user_id, REFERRER_BONUS_PKR, `Referral bonus: ${referral.referred_email || referral.referred_name || `User #${userId}`}`, null]
    );
    await client.query(
        'INSERT INTO balance_adjustments (user_id, amount, reason, admin_id) VALUES ($1, $2, $3, $4)',
        [userId, REFERRED_WELCOME_BONUS_PKR, `Referral welcome bonus from ${referral.referrer_email || referral.referrer_name || `User #${referral.referrer_user_id}`}`, null]
    );

    await client.query(
        'INSERT INTO transactions (user_id, user_email, amount, type, status, description) VALUES ($1, $2, $3, $4, $5, $6)',
        [referral.referrer_user_id, referral.referrer_email, REFERRER_BONUS_PKR, 'referral_referrer_bonus', 'approved', `Referral bonus from ${referral.referred_email || referral.referred_name || `User #${userId}`}`]
    );
    await client.query(
        'INSERT INTO transactions (user_id, user_email, amount, type, status, description) VALUES ($1, $2, $3, $4, $5, $6)',
        [userId, referral.referred_email, REFERRED_WELCOME_BONUS_PKR, 'referral_welcome_bonus', 'approved', `Referral welcome bonus from ${referral.referrer_email || referral.referrer_name || `User #${referral.referrer_user_id}`}`]
    );

    await client.query(`
        UPDATE referrals
        SET status = 'bonus_issued',
            blocked_reason = NULL,
            fraud_flags = $1,
            qualifying_deposit_amount = $2,
            qualifying_transaction_id = $3,
            qualifying_payment_request_id = $4,
            referrer_bonus = $5,
            referred_bonus = $6,
            bonus_issued = TRUE,
            bonus_issued_at = CURRENT_TIMESTAMP,
            updated_at = CURRENT_TIMESTAMP
        WHERE id = $7
    `, [
        fraudFlagsList.length ? fraudFlagsList.join(', ') : null,
        depositAmount,
        transactionId || null,
        paymentRequestId || null,
        REFERRER_BONUS_PKR,
        REFERRED_WELCOME_BONUS_PKR,
        referral.id
    ]);
    ...
    return {
        applied: true,
        status: 'bonus_issued',
        referrerBonus: REFERRER_BONUS_PKR,
        referredBonus: REFERRED_WELCOME_BONUS_PKR
    };
}
...
const otpQualityMeta = {
    excellent: { label: 'Excellent OTP', color: 'green' },
    good: { label: 'Good OTP', color: 'green' },
    average: { label: 'Average OTP', color: 'yellow' },
    low: { label: 'Low OTP', color: 'red' }
};

const serviceDefaultOtpQuality = {
    whatsapp: 'good',
    facebook: 'good',
    instagram: 'good',
    google: 'good',
...
function normalizeOtpQualityLevel(value) {
    const normalized = String(value || '').trim().toLowerCase();
    return otpQualityMeta[normalized] ? normalized : '';
}

function resolveConfiguredOtpQuality(configuredValue) {
    if (!configuredValue) return null;
    if (typeof configuredValue === 'string') {
        const normalizedQuality = normalizeOtpQualityLevel(configuredValue);
        if (!normalizedQuality) return null;
        return { ...otpQualityMeta[normalizedQuality] };
    }
    if (typeof configuredValue === 'object') {
        const normalizedQuality = normalizeOtpQualityLevel(configuredValue.quality);
        if (normalizedQuality) {
            return { ...otpQualityMeta[normalizedQuality] };
        }
        const label = String(configuredValue.label || '').trim();
        const color = String(configuredValue.color || '').trim().toLowerCase();
...
function ensureEditableOtpQualityServiceConfig(serviceType) {
    const normalizedServiceType = String(serviceType || '').trim().toLowerCase();
    if (!normalizedServiceType) return;
    serviceDefaultOtpQuality[normalizedServiceType] = normalizeOtpQualityLevel(serviceDefaultOtpQuality[normalizedServiceType]) || 'good';
    const serviceMap = countryQuality[normalizedServiceType];
    if (!serviceMap || typeof serviceMap !== 'object' || Array.isArray(serviceMap)) {
        countryQuality[normalizedServiceType] = {};
    }
}

function ensureEditableOtpQualityConfigForCatalog(catalog) {
    Object.keys(catalog || {}).forEach((serviceType) => {
        ensureEditableOtpQualityServiceConfig(serviceType);
    });
}
...
ensureEditableOtpQualityConfigForCatalog(serviceCatalog);
...
function getCountryQualityStatus(serviceType, countryName) {
    const normalizedServiceType = String(serviceType || '').trim().toLowerCase();
    if (!normalizedServiceType) return null;
    const hasKnownService = Object.prototype.hasOwnProperty.call(serviceCatalog, normalizedServiceType)
        || Object.prototype.hasOwnProperty.call(countryQuality, normalizedServiceType)
        || Object.prototype.hasOwnProperty.call(serviceDefaultOtpQuality, normalizedServiceType);
    if (!hasKnownService) return null;
    ensureEditableOtpQualityServiceConfig(normalizedServiceType);
    const normalizedCountryName = normalizeCountryNameForLookup(countryName);
    const serviceMap = countryQuality[normalizedServiceType];
    const configuredValue = normalizedCountryName ? serviceMap[normalizedCountryName] : undefined;
    const configuredStatus = resolveConfiguredOtpQuality(configuredValue);
    if (configuredStatus) return configuredStatus;
    const defaultQuality = normalizeOtpQualityLevel(serviceDefaultOtpQuality[normalizedServiceType]) || 'good';
    return { ...otpQualityMeta[defaultQuality] };
}

async function decorateCountriesWithOtpQuality(serviceType, countries) {
    const normalizedServiceType = String(serviceType || '').trim().toLowerCase();
    const isWhatsApp = normalizedServiceType === 'whatsapp';
    const baseCountries = isWhatsApp
    ...
    return baseCountries.map((country, index) => {
        ...
        if (isWhatsApp) {
            const manualStatus = getCountryQualityStatus(normalizedServiceType, nextCountry.name);
            if (!manualStatus) return nextCountry;
            return { ...nextCountry, statusLabel: manualStatus.label, statusColor: manualStatus.color };
        }
        ensureEditableOtpQualityServiceConfig(normalizedServiceType);
        const normalizedCountryName = normalizeCountryNameForLookup(nextCountry.name);
        const serviceMap = countryQuality[normalizedServiceType] || {};
        if (normalizedCountryName && Object.prototype.hasOwnProperty.call(serviceMap, normalizedCountryName)) {
            const manualStatus = resolveConfiguredOtpQuality(serviceMap[normalizedCountryName]);
            if (manualStatus) return { ...nextCountry, statusLabel: manualStatus.label, statusColor: manualStatus.color };
        }
        if (index >= 3) return nextCountry;
        return { ...nextCountry, statusLabel: otpQualityMeta.good.label, statusColor: otpQualityMeta.good.color };
    });
}
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    try {
        ...
        console.info(`[PASSWORD_RESET][request] email=${email}`);
        const user = await findUser(email);
        if (user) {
            const token = crypto.randomBytes(32).toString('hex');
            const expiresAt = new Date(Date.now() + PASSWORD_RESET_TOKEN_TTL_MS).toISOString();
            console.info(`[PASSWORD_RESET][user-found] userId=${user.id} email=${user.email}`);
            await savePasswordResetToken(user.id, hashToken(token), expiresAt);
            console.info(`[PASSWORD_RESET][token-saved] userId=${user.id} expiresAt=${expiresAt}`);
            try {
                await sendPasswordResetEmail(user, token, req);
            } catch (err) {
                await clearPasswordResetToken(user.id);
                console.error(`[PASSWORD_RESET][mail-failed] userId=${user.id} email=${user.email} | ${formatSmtpDebugError(err)}`);
                throw err;
            }
        } else {
            console.info(`[PASSWORD_RESET][user-missing] email=${email}`);
        }
        res.json({ success: true, message: 'If an account exists for that email, a reset link has been sent.' });
    } catch (err) {
        console.error(`[PASSWORD_RESET][request-failed] email=${email || 'unknown'} | ${formatSafeError(err, 'Could not send password reset email')}`);
...
app.get('/api/reset-password/validate', async (req, res) => {
    try {
        ...
        res.status(500).send(formatSafeError(err, 'Could not validate reset link'));
    }
});
...
app.get('/api/referral-program', ensureAuth, async (req, res) => {
    try {
        const summary = await getReferralProgramSummary(req.session.userId, getRequestBaseUrl(req));
        res.json(summary);
    } catch (err) {
...
app.get('/api/admin/referrals', ensureAdmin, async (_req, res) => {
    try {
        const referrals = await getAdminReferrals();
        res.json(referrals);
    } catch (err) {
        res.status(500).send(formatSafeError(err, 'Could not load referrals'));
    }
});
...
app.post('/api/reset-password', async (req, res) => {
    try {
        ...
        console.info(`[PASSWORD_RESET][completed] userId=${user.id} email=${user.email}`);
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const referralCode = normalizeReferralCode(req.body.referralCode || req.session?.pendingReferralCode || '');
        const fingerprintContext = getRequestFingerprintContext(req);
        ...
        await createUser(name, email, password, {
            referralCode,
            signupIp: fingerprintContext.requestIp,
            deviceFingerprint: fingerprintContext.deviceFingerprint,
            browserFingerprint: fingerprintContext.browserFingerprint
        });
        if (req.session?.pendingReferralCode && referralCode && req.session.pendingReferralCode === referralCode) {
...
app.post('/api/request-payment', ensureAuth, upload.single('screenshot'), async (req, res) => {
    try {
        ...
        const senderAccount = normalizeSenderAccount(req.body.sender_account || req.body.senderAccount);
        const fingerprintContext = getRequestFingerprintContext(req);
        ...
        if (!senderAccount) return res.status(400).send('Sender Easypaisa account is required');
        ...
        await queryRun(
            ...
            [
                ...
                'pending',
                senderAccount,
                fingerprintContext.requestIp || null,
                fingerprintContext.deviceFingerprint || null,
                fingerprintContext.browserFingerprint || null
            ]
        );
...
app.post('/api/admin/payment-requests/:id/approve', ensureAdmin, async (req, res) => {
    ...
    try {
        ...
        const transactionInsertRes = await client.query(
            `
            ...
            [
                ...
                null,
                paymentRequest.sender_account || null,
        ...
        await processReferralBonusForDeposit(client, {
            ...
            senderAccount: paymentRequest.sender_account,
            requestIp: paymentRequest.request_ip,
            deviceFingerprint: paymentRequest.device_fingerprint,
            browserFingerprint: paymentRequest.browser_fingerprint,
            transactionId: transactionInsertRes.rows[0]?.id || null,
