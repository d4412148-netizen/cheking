...
function hashFingerprint(value) {
    return hashToken(value).slice(0, 64);
}
...
async function evaluateReferralFraudRisk(client, paymentRequest, referredUser) {
    if (!referredUser || !referredUser.referred_by_user_id) {
        return { blocked: true, reason: 'missing_referral' };
    }
    const referrerRes = await client.query(
        'SELECT id, signup_ip, signup_device_id, signup_browser_fingerprint FROM users WHERE id = $1',
        [referredUser.referred_by_user_id]
    );
    const referrer = referrerRes.rows[0];
    if (!referrer) return { blocked: true, reason: 'invalid_referrer' };
    if (Number(referrer.id) === Number(referredUser.id)) return { blocked: true, reason: 'self_referral' };

    const referredIp = String(referredUser.signup_ip || '').trim();
    const referrerIp = String(referrer.signup_ip || '').trim();
    if (referredIp && referrerIp && referredIp === referrerIp) return { blocked: true, reason: 'same_ip' };

    const referredDevice = String(referredUser.signup_device_id || '').trim();
    const referrerDevice = String(referrer.signup_device_id || '').trim();
    if (referredDevice && referrerDevice && referredDevice === referrerDevice) return { blocked: true, reason: 'same_device' };

    const referredFingerprint = String(referredUser.signup_browser_fingerprint || '').trim();
    const referrerFingerprint = String(referrer.signup_browser_fingerprint || '').trim();
    if (referredFingerprint && referrerFingerprint && referredFingerprint === referrerFingerprint) {
        return { blocked: true, reason: 'same_browser_fingerprint' };
    }

    const paymentAccountRef = String(paymentRequest.payment_account_ref || paymentRequest.transaction_id || '').trim();
    if (paymentAccountRef) {
        const duplicateAccountRes = await client.query(
            `SELECT id FROM payment_requests
             WHERE payment_account_ref = $1
               AND user_id = $2
               AND status = 'approved'
             LIMIT 1`,
            [paymentAccountRef, referrer.id]
        );
        if (duplicateAccountRes.rows.length > 0) return { blocked: true, reason: 'duplicate_payment_account' };
    }

    return { blocked: false, referrerUserId: Number(referrer.id) };
}
...
async function initDB() {
    ...
    await queryRun('ALTER TABLE users ADD COLUMN IF NOT EXISTS signup_device_id TEXT');
    await queryRun('ALTER TABLE users ADD COLUMN IF NOT EXISTS signup_browser_fingerprint TEXT');
    await queryRun('ALTER TABLE users ADD COLUMN IF NOT EXISTS is_suspicious_referral BOOLEAN DEFAULT FALSE');
    ...
    `);
    await queryRun('ALTER TABLE payment_requests ADD COLUMN IF NOT EXISTS payment_account_ref TEXT');
    await queryRun('CREATE INDEX IF NOT EXISTS idx_payment_requests_payment_account_ref ON payment_requests (payment_account_ref)');
    ...
    `);
    await queryRun('CREATE INDEX IF NOT EXISTS idx_referral_rewards_referrer_user_id ON referral_rewards (referrer_user_id, created_at DESC)');
    await queryRun('CREATE INDEX IF NOT EXISTS idx_referral_rewards_payment_request_id ON referral_rewards (payment_request_id)');
...
async function createUser(name, email, password, options = {}) {
    ...
    const referredByUserId = options.referredByUserId == null ? null : Number(options.referredByUserId);
    const signupIp = String(options.signupIp || '').trim() || null;
    const signupDeviceId = String(options.signupDeviceId || '').trim() || null;
    const signupBrowserFingerprint = String(options.signupBrowserFingerprint || '').trim() || null;
    return queryRun(
        'INSERT INTO users (email, password, name, referral_code, referred_by_user_id, signup_ip, signup_device_id, signup_browser_fingerprint) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
        [sanitizeEmail(email), hashedPassword, String(name || '').trim(), referralCode, referredByUserId, signupIp, signupDeviceId, signupBrowserFingerprint]
    );
}
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const browserFingerprint = hashFingerprint(req.body.browserFingerprint).slice(0, 64);
        ...
        if (referralCode) {
            const referrer = await queryOne('SELECT id FROM users WHERE referral_code = $1', [referralCode]);
            if (!referrer) return res.status(400).send('Invalid referral code');
        ...
        await createUser(name, email, password, {
            ...
            signupBrowserFingerprint: browserFingerprint
        });
        res.json({ success: true });
    } catch (err) {
...
app.post('/api/request-payment', ensureAuth, upload.single('screenshot'), async (req, res) => {
    try {
        ...
        await queryRun(
            'INSERT INTO payment_requests (user_id, user_email, amount, transaction_id, screenshot, status, payment_account_ref) VALUES ($1, $2, $3, $4, $5, $6, $7)',
            [req.session.userId, user.email, amount, transaction_id || null, screenshot, 'pending', transaction_id || null]
        );
...
app.post('/api/admin/payment-requests/:id/approve', ensureAdmin, async (req, res) => {
    ...
    try {
        ...
        if (Number(paymentRequest.amount || 0) >= 170 && user.referred_by_user_id) {
            const alreadyRewardedRes = await client.query(
                'SELECT id FROM referral_rewards WHERE referred_user_id = $1 LIMIT 1',
                [user.id]
            );
            if (alreadyRewardedRes.rows.length === 0) {
                const fraudCheck = await evaluateReferralFraudRisk(client, paymentRequest, user);
                if (fraudCheck.blocked) {
                    await client.query(
                        'INSERT INTO referral_rewards (referrer_user_id, referred_user_id, payment_request_id, reward_amount, status, fraud_reason) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (referred_user_id) DO NOTHING',
                        [user.referred_by_user_id, user.id, paymentRequest.id, 50, 'blocked', fraudCheck.reason]
                    );
                    await client.query('UPDATE users SET is_suspicious_referral = TRUE WHERE id = $1', [user.id]);
                    await client.query('UPDATE users SET is_suspicious_referral = TRUE WHERE id = $1', [user.referred_by_user_id]);
                } else {
                    const REFERRER_BONUS = 50;
                    const REFERRED_BONUS = 20;
                    await client.query('UPDATE users SET balance = COALESCE(balance, 0) + $1 WHERE id = $2', [REFERRER_BONUS, fraudCheck.referrerUserId]);
                    await client.query('UPDATE users SET balance = COALESCE(balance, 0) + $1 WHERE id = $2', [REFERRED_BONUS, user.id]);
                    await client.query(
                        'INSERT INTO referral_rewards (referrer_user_id, referred_user_id, payment_request_id, reward_amount, status) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (referred_user_id) DO NOTHING',
                        [fraudCheck.referrerUserId, user.id, paymentRequest.id, REFERRER_BONUS, 'awarded']
                    );
                    const referrerEmailRes = await client.query('SELECT email FROM users WHERE id = $1', [fraudCheck.referrerUserId]);
                    await client.query(
                        'INSERT INTO transactions (user_id, user_email, amount, type, status, description, transaction_id, screenshot) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
                        [fraudCheck.referrerUserId, referrerEmailRes.rows[0]?.email || '', REFERRER_BONUS, 'referral_bonus', 'approved', `Referral bonus from user #${user.id}`, `REF-BONUS-${paymentRequest.id}`, null]
                    );
                    await client.query(
                        'INSERT INTO transactions (user_id, user_email, amount, type, status, description, transaction_id, screenshot) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
                        [user.id, user.email || paymentRequest.user_email || '', REFERRED_BONUS, 'referral_join_bonus', 'approved', 'Referral join bonus (first 170+ deposit)', `REF-JOIN-${paymentRequest.id}`, null]
                    );
                }
            }
        }
