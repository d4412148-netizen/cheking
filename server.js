...
const microsoftCountries = [
    ...
    { name: 'Indonesia', code: '+62', price: 10, countryId: 6, flag: '🇮🇩' }
];
...
const INDONESIA_COUNTRY = { name: 'Indonesia', code: '+62', price: 10, countryId: 6, flag: '🇮🇩' };
...
Object.entries(additionalServiceMap).forEach(([code, name]) => {
    const key = code.toLowerCase().replace(/[^a-z0-9_]/g, '_');
    if (!serviceCatalog[key]) {
        serviceCatalog[key] = {
            serviceType: key,
            serviceName: name + ' Number',
            serviceCode: code,
            countries: [{ ...INDONESIA_COUNTRY }]
        };
    }
});
