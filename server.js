...
function createMailTransporter() {
    ...
    try {
        ...
        const transportConfig = {
            ...(SMTP_URL ? { url: SMTP_URL } : {}),
            ...(!SMTP_URL && (SMTP_SERVICE || inferredSmtpService) ? { service: SMTP_SERVICE || inferredSmtpService } : {}),
            ...(!SMTP_URL && !(SMTP_SERVICE || inferredSmtpService) && SMTP_HOST ? {
                host: SMTP_HOST,
                port: SMTP_PORT,
                secure: SMTP_SECURE
            } : {}),
...
let mailTransporterReadyPromise = null;

function ensureMailTransporterReady() {
    if (!mailTransporter) {
    ...
    if (!mailTransporterReadyPromise) {
        mailTransporterReadyPromise = mailTransporter.verify()
            .then(() => true)
            .catch((err) => {
                mailTransporterReadyPromise = null;
                throw err;
            });
    }
    return mailTransporterReadyPromise;
}
