...
function getServiceDomainForLogo(serviceValue) {
    ...
    const fallbackToken = candidates.find((candidate) => /^[a-z0-9]+$/.test(candidate) && candidate.length > 1);
    if (!fallbackToken) return '';
    if (fallbackToken === 'x') return 'x.com';
    return `${fallbackToken}.com`;
}
...
function resolveCachedLogoPublicPath(baseName) {
    ...
    for (const extension of supportedExtensions) {
        const logoFilePath = path.join(LOGO_CACHE_DIR, `${baseName}${extension}`);
        if (fs.existsSync(logoFilePath)) {
            return `/logos/${baseName}${extension}`;
        }
    }
    return '';
}
...
async function downloadRemoteLogo(remoteUrl, baseName) {
    ...
    const targetPath = path.join(LOGO_CACHE_DIR, `${baseName}${extension}`);
